				...
				<tbody>
					<c:forEach items="${questionnaires}" var="questionnaryInfo">
						<tr>
							...
							<td><div class="dis-ellipsis pr-100"
									title="${fn:escapeXml(questionnaryInfo.title)}">${questionnaryInfo.title}</div></td>
					...
					if(questionnaireId != null && questionnaireId != '' && typeof questionnaireId !='undefined'){
						$.ajax({
			    			...
			    			success: function deleteConsentInfo(data){
			    				...
			    				if(status == "SUCCESS"){
			    					$("#alertMsg").removeClass('e-box').addClass('s-box').text("Questionnaire deleted successfully");
			    					$('#alertMsg').show();
			    					...
			    					if(status == 'FAILUREanchorused'){
			    						$("#alertMsg").removeClass('s-box').addClass('e-box').text("Questionnaire already live anchorbased.unable to delete");
			    					}else{
		  ...
		  $.each(questionnaireList, function(i, obj) {
             var datarow = [];
 			 ...
 			 if(typeof obj.title === "undefined" && typeof obj.title === "undefined" ){
					...
					datarow.push('<div class="dis-ellipsis pr-100" title='+DOMPurify.sanitize(obj.title)+'>' + DOMPurify.sanitize(obj.title) + '</div>');
			 }	
 			 ...
 			 var actionDiv = "<span class='sprites_icon preview-g mr-lg' data-toggle='tooltip' data-placement='top' title='View' onclick='viewQuestionnaires(" + parseInt(obj.id) + ");'></span>";
 			 if(obj.status){
 				actionDiv += "<span class='sprites_icon edit-g mr-lg' data-toggle='tooltip' data-placement='top' title='Edit' onclick='editQuestionnaires(" + parseInt(obj.id) + ");'></span>";
 			 }else{
 				actionDiv += "<span class='edit-inc-draft mr-md mr-lg' data-toggle='tooltip' data-placement='top' title='Edit' onclick='editQuestionnaires(" + parseInt(obj.id) + ");'></span>";
 			 }
 			 actionDiv += "<span class='sprites_icon copy  mr-lg' data-toggle='tooltip' data-placement='top' title='Copy' onclick='copyQuestionnaire(" + parseInt(obj.id) + ");'></span>";
 			 
 			 actionDiv += "<span class='sprites_icon copy delete' data-toggle='tooltip' data-placement='top' title='Delete' onclick='deleteQuestionnaire(" + parseInt(obj.id) + ");'></span>";
