			...
			<div class="col-md-12 p-none">
				...
				<div class="form-group">
					<span class="radio radio-info radio-inline p-45"> <input
						...
						id="inlineRadio4" value="No" name="retainParticipant"
						<c:if test="${studyBo.retainParticipant eq 'No'}">checked</c:if>
						required> <label for="inlineRadio4">No</label>
					</span> <span class="radio radio-inline"> <input type="radio"
						id="inlineRadio5" value="All" name="retainParticipant"
			...
			<div class="col-md-12 p-none">
				...
				<div class="form-group">
					<span class="radio radio-info radio-inline p-45"> <input
						...
						class="rejoin_radio" id="inlineRadio7" value="No"
						name="allowRejoin"
						<c:if test="${studyBo.allowRejoin eq 'No'}">checked</c:if>
						required> <label for="inlineRadio7">No</label>
					</span>
					<div class="help-block with-errors red-txt"></div>
				</div>
			...
			<c:if test="${fn:contains(permissions,7)}">
				<div>
					<div class="black-md-f text-uppercase line34">MANAGE LIST OF
						ADMINS ASSIGNED TO THE STUDY</div>
							...
							<c:forEach items="${studyPermissionList}" var="perm">
								<tr id="studyAdminRowId${perm.userId}" class="studyAdminRowCls"
									...
									<td><span class="dis-ellipsis"
										title="${fn:escapeXml(perm.userFullName)}">${perm.userFullName}</span></td>
			...
			<div class="text-right">
				...
				<div class="dis-line form-group mb-none mr-sm">
					<button type="button" class="btn btn-default gray-btn"
						onclick="cancelAddAdmin();">Cancel</button>
				</div>
				<div class="dis-line form-group mb-none mr-sm">
					<button type="button" class="btn btn-primary blue-btn"
						id="addAdminsToStudyId" onclick="addAdminsToStudy()">Add</button>
				</div>
			</div>
		</div>
		<div class="right-content-body col-xs-12">
					...
					<tbody>
						<c:forEach items="${userList}" var="user">
							<tr id="user${user.userId}" class="checkCount">
								...
								<td><span class="dis-ellipsis"
									title="${fn:escapeXml(user.userFullName)}">${user.userFullName}</span></td>
								<td><span class="dis-ellipsis"
									title="${fn:escapeXml(user.userEmail)}">${user.userEmail}</span></td>
								<td>${user.roleName}</td>
							</tr>
						</c:forEach>
					</tbody>
		...
		function addAdminsToStudy(){
			 ...
			 $('[name=case]:checked').each(function() {
				 ...
				 var domStr = '';
				      ...
				      domStr = domStr + '<td><span class="dis-ellipsis" title="' + DOMPurify.sanitize(name) + '">' + DOMPurify.sanitize(name) +'</span></td>';
