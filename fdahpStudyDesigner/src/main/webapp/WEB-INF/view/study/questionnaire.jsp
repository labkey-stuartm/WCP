				...
				success: function consentInfo(data){
					...
					if(status == "SUCCESS"){
					   ...
					   var isDone = data.isDone;
						...
						$('#alertMsg').show();
						$("#alertMsg").removeClass('s-box').addClass('e-box').text("Unable to reorder questionnaire");
...
function checkDateRange(){
	$(document).on('dp.change change', '.cusStrDate, .cusEndDate, .cusTime', function(e) {
		...
		if(!chkVal) {
			$(thisAttr).parents('.manually-option').find('.cusTime').parent().addClass('has-error has-danger').find(".help-block").removeClass('with-errors').empty().append(
			...
			$(thisAttr).parents('.manually-option').find('.cusTime').parent().removeClass('has-error has-danger').find(".help-block").addClass('with-errors').empty();
		}	
		var a = 0;
		    			...
		    			success: function deleteConsentInfo(data){
		    				...
		    				if(status == "SUCCESS"){
		    					...
		    					if(status == 'FAILUREanchorused'){
		    						$("#alertMsg").removeClass('s-box').addClass('e-box').text("Questionnaire step already live anchorbased.unable to delete");
		    					}else{
	 ...
	 if(typeof questionnaire != 'undefined' && questionnaire != null && Object.keys(questionnaire).length > 0){
		 $.each(questionnaire, function(key, value) {
			 ...
			 if(typeof key === "undefined"){
			     ...
			     if($("#branchingId").is(':checked')){
			    	 datarow.push('<div class="destinationStep questionnaireStepClass" style="display: block;">' + DOMPurify.sanitize(value.destinationText) + '</div>');
			     }else{
			    	 datarow.push('<div class="destinationStep questionnaireStepClass" style="display: none;">' + DOMPurify.sanitize(value.destinationText) + '</div>'); 
			     }
			     var dynamicAction ='<div>'+
			      ...
			      dynamicAction +='<span class="ellipse" onmouseenter="ellipseHover(this);"></span>'+
					              '<div class="ellipse-hover-icon" onmouseleave="ellipseUnHover(this);">'+
					               '  <span class="sprites_icon preview-g mr-sm" onclick="viewStep(' + parseInt(value.stepId) + ',&#34;' + DOMPurify.sanitize(value.stepType) + '&#34;)"></span>';
				  if(value.status){
					  dynamicAction += '<span class="sprites_icon edit-g mr-sm" onclick="editStep(' + parseInt(value.stepId) + ',&#34;' + DOMPurify.sanitize(value.stepType) + '&#34;)"></span>';
				  }else{
					  dynamicAction += '<span class="edit-inc-draft mr-md mr-sm" onclick="editStep(' + parseInt(value.stepId) + ',&#34;' + DOMPurify.sanitize(value.stepType) + '&#34;)"></span>';
				  }
				  dynamicAction += '  <span class="sprites_icon delete deleteStepButton" onclick="deletStep(' + parseInt(value.stepId) + ',&#34;' + DOMPurify.sanitize(value.stepType) + '&#34;)"></span>'+
					              '</div>'+
					           '</div>';
...
function chkDaysValid(clickDone){
	...
	if(y && x){
		if(parseInt(x) > parseInt(y)){
			...
			$('#onetimeydaysId').parent().addClass('has-error has-danger').find(".help-block").empty().append($("<ul><li> </li></ul>").attr("class","list-unstyled").text(
...
//		    $("#"+i).remove();
		...
		if(ydayValue !== ""){
		  if(xdayValue > ydayValue){		    
		    ...
		    $("#ydays"+parent_id).parent().addClass('has-error has-danger').find(".help-block").empty().append($("<ul><li> </li></ul>").attr("class","list-unstyled").text(
            "Y days should be greater than X days."));
		  ...
		  if(xdayValue < pydayValue){
			  ...
			  $(this).parent().addClass('has-error has-danger').find(".help-block").empty().append($("<ul><li> </li></ul>").attr("class","list-unstyled").text(
		...
		jQuery(document).on("change",".sign-box select",function(){
			...
			if($('.manually-anchor-option').length > 1){
				  ...
				  if(xdayValue < pydayValue){
					  $(this).addClass("red-border");
					  $("#ydays"+pre_parent).addClass("red-border");
					  $("#xdays"+parent_id).parent().addClass('has-error has-danger').find(".help-block").empty().append($("<ul><li> </li></ul>").attr("class","list-unstyled").text(
		              "Child X days should be greater than parent Y days."));
					  $("#addbtn"+parent_id).addClass("not-allowed");
				  }else{
					  $(this).removeClass("red-border");
