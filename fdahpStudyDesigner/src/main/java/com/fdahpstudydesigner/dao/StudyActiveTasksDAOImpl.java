...
public class StudyActiveTasksDAOImpl implements StudyActiveTasksDAO {
    ...
    private static final Logger logger = Logger.getLogger(StudyActiveTasksDAOImpl.class.getName());
    ...
    public String deleteActiveTask(ActiveTaskBo activeTaskBo, SessionObject sesObj, String customStudyId) {
        ...
        try {
            ...
            if (activeTaskBo != null) {
                ...
                queryString = "DELETE From NotificationBO where activeTaskId=:activeId AND notificationSent=false";
                session.createQuery(queryString).setInteger("activeId", activeTaskBo.getId()).executeUpdate();
                ...
                if (studyVersionBo != null) {
                    ...
                    activityDetails = "Active Task was deactivated. (Active Task Key = " + activeTaskBo.getShortTitle()
                            + ", Study ID = " + customStudyId + ")";

                    deleteActQuery = "update ActiveTaskAtrributeValuesBo set active=0 where activeTaskId= :activeTaskId";
                    query = session.createQuery(deleteActQuery).setInteger("activeTaskId", activeTaskBo.getId());
                    query.executeUpdate();

                    deleteQuery = "update ActiveTaskBo set active=0 ,modifiedBy=:userId"
                            + ",modifiedDate=:currentDateTime ,customStudyId=:customStudyId where id=:activeTaskId";
                    query = session.createQuery(deleteQuery).setInteger("userId", sesObj.getUserId())
                            .setString("currentDateTime", FdahpStudyDesignerUtil.getCurrentDateTime())
                            .setString("customStudyId", customStudyId).setInteger("activeTaskId", activeTaskBo.getId());
                    result = query.executeUpdate();
                } else {
                    // hard delete active task before study launch
                    session.createSQLQuery("DELETE FROM active_task_frequencies WHERE active_task_id=:activeTaskId")
    ...
    public ActiveTaskBo getActiveTaskById(Integer activeTaskId, String customStudyId) {
        ...
        try {
            ...
            if (activeTaskBo != null) {
                query = session.createQuery("from ActiveTaskAtrributeValuesBo where activeTaskId=:activeTaskId")
                ...
                if (StringUtils.isNotEmpty(customStudyId)) {
                    ...
                    BigInteger shortTitleCount = (BigInteger) session
                            .createSQLQuery("select count(*) from active_task a "
                                    + "where a.short_title=:title and custom_study_id=:customStudyId"
                                    + " and a.active=1 and a.is_live=1")
                            .setString("title", activeTaskBo.getShortTitle()).setString("customStudyId", customStudyId)
                            .uniqueResult();
                    ...
                    for (ActiveTaskAtrributeValuesBo activeTaskAtrributeValuesBo : activeTaskAtrributeValuesBos) {
                        if (StringUtils.isNotEmpty(customStudyId)) {
                            ...
                            BigInteger statTitleCount = (BigInteger) session
                                    .createSQLQuery("select count(*) from active_task_attrtibutes_values at "
                                            ...
                                            + " and a.active=1 and a.is_live=1)")
                                    .setString("customStudyId", customStudyId).setString("identifierNameStat",
    ...
    public List<ActiveTaskBo> getStudyActiveTasksByStudyId(String studyId, Boolean isLive) {
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(studyId)) {
                if (isLive) {
                    String searchQuery = "SELECT ATB FROM ActiveTaskBo ATB where ATB.active IS NOT NULL and ATB.active=1 and ATB.customStudyId =:studyId"
                            + " and ATB.live=1 order by id";
                    query = session.createQuery(searchQuery).setParameter("studyId", studyId);
                } else {
                    query = session.getNamedQuery("ActiveTaskBo.getActiveTasksByByStudyId").setInteger("studyId",
                ...
                if (activeTasks != null && !activeTasks.isEmpty() && activeTaskListBos != null
                    ...
                    for (ActiveTaskBo activeTaskBo : activeTasks) {
                        if (activeTaskBo.getTaskTypeId() != null) {
                            for (ActiveTaskListBo activeTaskListBo : activeTaskListBos) {
                                if (activeTaskListBo.getActiveTaskListId().intValue() == activeTaskBo.getTaskTypeId()
                                        .intValue()) {
     ...
     *
     * @param activeTaskBo  , {@link ActiveTaskBo}
     * @param customStudyId , the custom id of study
     * @return {@link ActiveTaskBo}
     */
    @Override
    public ActiveTaskBo saveOrUpdateActiveTask(ActiveTaskBo activeTaskBo, String customStudyId) {
        ...
        try {
            ...
            if (null != activeTaskBo && null != activeTaskBo.getAnchorDateId()) {
                anchorDateTypeBo = (AnchorDateTypeBo) session.get(AnchorDateTypeBo.class,
                        activeTaskBo.getAnchorDateId());
            }
            if (null != anchorDateTypeBo && null != anchorDateTypeBo.getParticipantProperty()
                ...
                query = session.createQuery(
                        ...
                        .setInteger("anchorDateId", activeTaskBo.getAnchorDateId());
                Long count = (Long) query.uniqueResult();
                if (count < 1) {
                    query = session.createQuery(
            ...
            if (activeTaskBo.getType().equalsIgnoreCase(FdahpStudyDesignerConstants.SCHEDULE) && activeTaskBo != null
                ...
                if (activeTaskBo.getActiveTaskFrequenciesList() != null
                    ...
                    query = session.createSQLQuery(deleteQuery2).setInteger("activeTaskId", activeTaskBo.getId());
                    query.executeUpdate();
                    for (ActiveTaskFrequencyBo activeTaskFrequencyBo : activeTaskBo.getActiveTaskFrequenciesList()) {
                        if (activeTaskFrequencyBo.getFrequencyTime() != null) {
                            activeTaskFrequencyBo.setFrequencyTime(FdahpStudyDesignerUtil.getFormattedDate(
                                    activeTaskFrequencyBo.getFrequencyTime(), FdahpStudyDesignerConstants.SDF_TIME,
                ...
                if (activeTaskBo.getActiveTaskFrequenciesList() != null) {
                    ...
                    if (activeTaskFrequencyBo.getFrequencyDate() != null
                            || activeTaskFrequencyBo.getFrequencyTime() != null || activeTaskBo.getFrequency()
                        ...
                        if (activeTaskBo.getActiveTaskFrequenciesBo().getFrequencyDate() != null
                            ...
                            activeTaskFrequencyBo.setFrequencyDate(FdahpStudyDesignerUtil.getFormattedDate(
                    ...
                    for (ActiveTaskCustomScheduleBo activeTaskCustomScheduleBo : activeTaskBo
                        ...
                        if (activeTaskCustomScheduleBo.getFrequencyTime() != null) {
                            ...
                            if (activeTaskCustomScheduleBo.getFrequencyStartDate() != null
                                ...
                                activeTaskCustomScheduleBo.setFrequencyStartDate(FdahpStudyDesignerUtil
                                        .getFormattedDate(activeTaskCustomScheduleBo.getFrequencyStartDate(),
                                                FdahpStudyDesignerConstants.UI_SDF_DATE,
                                                FdahpStudyDesignerConstants.SD_DATE_FORMAT));
                            }
                            if (activeTaskCustomScheduleBo.getFrequencyEndDate() != null
                                    && !activeTaskCustomScheduleBo.getFrequencyEndDate().isEmpty()) {
                                activeTaskCustomScheduleBo.setFrequencyEndDate(FdahpStudyDesignerUtil.getFormattedDate(
                            ...
                            if (activeTaskCustomScheduleBo.getFrequencyTime() != null
                                ...
                                activeTaskCustomScheduleBo.setFrequencyTime(FdahpStudyDesignerUtil.getFormattedDate(
                            ...
                            if (activeTaskCustomScheduleBo.getTimePeriodFromDays() != null) {
                                activeTaskCustomScheduleBo
                                        .setTimePeriodFromDays(activeTaskCustomScheduleBo.getTimePeriodFromDays());
                            }
                            ...
                            if (activeTaskCustomScheduleBo.getTimePeriodToDays() != null) {
                                activeTaskCustomScheduleBo
                                        .setTimePeriodToDays(activeTaskCustomScheduleBo.getTimePeriodToDays());
                            }
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(activeTaskBo.getButtonText())) {
                studySequence = (StudySequenceBo) session.getNamedQuery("getStudySequenceByStudyId")
                        .setInteger("studyId", activeTaskBo.getStudyId()).uniqueResult();
            ...
            if (activeTaskBo.getButtonText().equalsIgnoreCase(FdahpStudyDesignerConstants.ACTION_TYPE_SAVE)) {
                ...
                activitydetails = "Content saved for Active Task. (Active Task Key  = " + activeTaskBo.getShortTitle()
                        + ",  Study ID = " + customStudyId + ")";
            } else {
                ...
                activitydetails = "Active Task succesfully checked for minimum content completeness and marked 'Done'. (Active Task Key = "
                        + activeTaskBo.getShortTitle() + ", Study ID =" + customStudyId + ")";
                auditLogDAO.updateDraftToEditedStatus(session, transaction, sesObj.getUserId(),
                        FdahpStudyDesignerConstants.DRAFT_ACTIVETASK, activeTaskBo.getStudyId());
                ...
                if (studyBo != null) {
                    ...
                    notificationBO = (NotificationBO) session.createQuery(queryString)
                            .setInteger("activeTaskId", activeTaskBo.getId()).uniqueResult();
                ...
                if (activeTaskAttName.equalsIgnoreCase(FdahpStudyDesignerConstants.SHORT_NAME_STATISTIC)) {
                    if (customStudyId != null && !customStudyId.isEmpty()) {
                        ...
                        if (activeTaskAtrributeValuesBos != null && !activeTaskAtrributeValuesBos.isEmpty()) {
                            ...
                            // questionnaire
                            queryString = "From QuestionsBo QBO where QBO.id IN (select QSBO.instructionFormId from QuestionnairesStepsBo QSBO where QSBO.questionnairesId IN (select id from QuestionnaireBo Q where Q.studyId in(select id From StudyBo SBO WHERE customStudyId= :customStudyId"
                                    + ")) and QSBO.stepType=:type) and QBO.statShortName=:activeTaskAttIdVal";
                            query = session.createQuery(queryString)
                                    .setString("type", FdahpStudyDesignerConstants.QUESTION_STEP)
                            ...
                            if (questionnairesStepsBo != null && !questionnairesStepsBo.isEmpty()) {
                                ...
                                BigInteger subCount = (BigInteger) session.createSQLQuery(queryString)
                                        .setString("customStudyId", customStudyId)
                                        .setString("activeTaskAttIdVal", activeTaskAttIdVal).uniqueResult();
								flag = subCount != null && subCount.intValue() > 0;
                            }
                        }
                    } else {
                        if (!activeTaskAttIdName.equals("static")) {
                        ...
                        activeTaskAtrributeValuesBos = session.createQuery(queryString)
                                .setParameterList("activeTaskAttIdName", Arrays.asList(activeTaskAttIdName))
                                .setInteger("studyId", studyId).setString("activeTaskAttIdVal", activeTaskAttIdVal)
                                .list();
                        if (activeTaskAtrributeValuesBos != null && !activeTaskAtrributeValuesBos.isEmpty()) {
                            flag = true;
                        } else {
                            // to check chart short title exist in question of
                            // questionnaire
                            queryString = "From QuestionsBo QBO where QBO.id IN (select QSBO.instructionFormId from QuestionnairesStepsBo QSBO where QSBO.questionnairesId IN (select id from QuestionnaireBo Q where Q.studyId=:studyId"
                                    + ") and QSBO.stepType=:type) and QBO.statShortName=:activeTaskAttIdVal";
                            query = session.createQuery(queryString)
                                    .setString("type", FdahpStudyDesignerConstants.QUESTION_STEP)
                                    .setInteger("studyId", studyId).setString("activeTaskAttIdVal", activeTaskAttIdVal);
                    ...
                    if (customStudyId != null && !customStudyId.isEmpty()) {
                        ...
                        taskBos = session.createQuery(queryString).setString("customStudyId", customStudyId)
                                .setString("activeTaskAttIdVal", activeTaskAttIdVal).list();
                        if (taskBos != null && !taskBos.isEmpty()) {
							...
							flag = questionnaireBo != null && !questionnaireBo.isEmpty();
                        }
                    } else {
                        // to check short title exist in active task or not
                        queryString = "from ActiveTaskBo where studyId=:studyId and shortTitle=:activeTaskAttIdVal";
                        taskBos = session.createQuery(queryString).setInteger("studyId", studyId)
                                .setString("activeTaskAttIdVal", activeTaskAttIdVal).list();
                        if (taskBos != null && !taskBos.isEmpty()) {
                            flag = true;
                        } else {
                            // to check short title exist in questionnaire or
                            // not
                            questionnaireBo = session.getNamedQuery("checkQuestionnaireShortTitle")
                                    .setInteger("studyId", studyId).setString("shortTitle", activeTaskAttIdVal).list();
							flag = questionnaireBo != null && !questionnaireBo.isEmpty();
                        }
                    }
                }
     ...
     * @author BTC
     * @param String                , customStudyId
     * @param activeStatisticsBeans , {@link List<ActiveStatisticsBean>}
     * @throws IOException
    ...
    public List<ActiveStatisticsBean> validateActiveTaskStatIds(String customStudyId,
        ...
        try {
            session = hibernateTemplate.getSessionFactory().openSession();
            if (activeStatisticsBeans != null && !activeStatisticsBeans.isEmpty()
                ...
                for (ActiveStatisticsBean activeStatisticsBean : activeStatisticsBeans) {
                    if (!activeStatisticsBean.getDbVal().equalsIgnoreCase(activeStatisticsBean.getIdVal())) {
                        queryString = "from ActiveTaskAtrributeValuesBo where activeTaskId in(select id from ActiveTaskBo where studyId IN "
                                ...
                                + ")) and identifierNameStat =:identifierNameStat" + subString;
                        activeTaskAtrributeValuesBos = session.createQuery(queryString).setParameterList("ids", ids)
                                .setString("customStudyId", customStudyId)
                                .setString("identifierNameStat", activeStatisticsBean.getIdVal()).list();
                        if (activeTaskAtrributeValuesBos != null && !activeTaskAtrributeValuesBos.isEmpty()) {
