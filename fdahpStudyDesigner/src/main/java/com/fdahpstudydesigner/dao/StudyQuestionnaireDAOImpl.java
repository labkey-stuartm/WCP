...
public class StudyQuestionnaireDAOImpl implements StudyQuestionnaireDAO {
	 ...
	 * server so its should be unique across all the steps.Validating the Unique of question short title inside form step
	 * 
	 * @author BTC
	 * @param String
	...
	public String checkFromQuestionShortTitle(Integer questionnaireId,
		...
		try {
			...
			if (questionnaireShortTitle != null
					&& !questionnaireShortTitle.isEmpty()) {
				query = session
						.createQuery("From QuestionnairesStepsBo QSBO where QSBO.questionnairesId IN (select QBO.id From QuestionnaireBo QBO where QBO.shortTitle='"
								+ questionnaireShortTitle
								+ "' and QBO.studyId in(select id From StudyBo SBO WHERE customStudyId='"
								+ customStudyId
								+ "')) and QSBO.stepShortTitle='"
								+ shortTitle
								+ "'");
				...
				if (questionnairesStepsBo != null
					...
					String searchQuery = "select count(*) From questions QBO,form_mapping f,questionnaires_steps QSBO,questionnaires Q where QBO.id=f.question_id "
							...
							+ questionnaireShortTitle
							+ "' and Q.study_id in(select id From studies SBO WHERE custom_study_id='"
							+ customStudyId
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - checkQuestionnaireStepShortTitle() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	/**
	 * This method is used to validate the questionnaire have response type text scale while changing the platform in study settings page
	 * 
	 * @author BTC
	 * @param Integer
	...
	public String checkQuestionnaireResponseTypeValidation(Integer studyId,
		...
		try {
			...
			String searchQuery = "select count(*) from questions QBO,questionnaires_steps QSBO,questionnaires Q where QBO.id=QSBO.instruction_form_id"
					+ " and QSBO.questionnaires_id=Q.id and Q.study_id="
					+ studyId
					+ " and Q.active=1 and QSBO.step_type='"
			...
			if (count != null && count.intValue() > 0) {
				...
				questionCount = (BigInteger) session.createSQLQuery(
						searchQuuery).uniqueResult();
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - checkQuestionnaireResponseTypeValidation() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public String checkQuestionnaireShortTitle(Integer studyId,
		...
		try {
			...
			if (customStudyId != null && !customStudyId.isEmpty()) {
				...
				query = session
						.createQuery("From QuestionnaireBo QBO where QBO.studyId IN(select id From StudyBo SBO WHERE customStudyId='"
								+ customStudyId
								+ "') and QBO.shortTitle='"
								+ shortTitle + "'");
				...
				query = session.getNamedQuery("checkQuestionnaireShortTitle")
				...
				if (questionnaireBo != null && !questionnaireBo.isEmpty()) {
					...
					queryString = "from ActiveTaskBo where studyId=" + studyId
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - checkQuestionnaireShortTitle() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public String checkStatShortTitle(Integer studyId, String shortTitle,
		...
		try {
			...
			if (customStudyId != null && !customStudyId.isEmpty()) {
				...
				if (count != null && count.intValue() > 0) {
					...
					if (subCount != null && subCount.intValue() > 0) {
						...
						String taskQuery = "from ActiveTaskAtrributeValuesBo where activeTaskId in(select id from ActiveTaskBo where studyId IN(select id From StudyBo SBO WHERE customStudyId='"
								...
								+ shortTitle
								+ "'";
						List<ActiveTaskAtrributeValuesBo> activeTaskAtrributeValuesBos = session
								.createQuery(taskQuery).list();
				...
				query = session
						.createQuery("From QuestionsBo QBO where QBO.id IN (select QSBO.instructionFormId from QuestionnairesStepsBo QSBO where QSBO.questionnairesId IN (select id from QuestionnaireBo Q where Q.studyId="
								+ studyId
								+ ") and QSBO.stepType='"
								+ FdahpStudyDesignerConstants.QUESTION_STEP
								+ "' and QSBO.active=1) and QBO.statShortName='"
								+ shortTitle + "'");
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - checkStatShortTitle() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public QuestionnaireBo copyStudyQuestionnaireBo(Integer questionnaireId,
		...
		try {
			...
			query = session
					.createQuery(" From QuestionnaireBo QBO WHERE QBO.id="
							+ questionnaireId);
			...
			if (questionnaireBo != null) {
				...
				newQuestionnaireBo.setCreatedDate(FdahpStudyDesignerUtil
						.getCurrentDateTime());
					...
					for (QuestionnairesStepsBo questionnairesStepsBo : existedQuestionnairesStepsBoList) {
						...
						if (destionStep.equals(0)) {
							...
							for (int i = 0; i < existedQuestionnairesStepsBoList
									.size(); i++) {
								if (existedQuestionnairesStepsBoList.get(i)
										.getStepId() != null
										&& destionStep
						...
						if (StringUtils.isNotEmpty(questionnairesStepsBo
							...
							newQuestionnairesStepsBo
									.setCreatedOn(FdahpStudyDesignerUtil
											.getCurrentDateTime());
							newQuestionnairesStepsBo.setCreatedBy(sessionObject
									.getUserId());
							...
							if (questionnairesStepsBo
								...
								if (questionsBo != null) {
									...
									newQuestionsBo
											.setCreatedOn(FdahpStudyDesignerUtil
													.getCurrentDateTime());
									newQuestionsBo.setCreatedBy(sessionObject
											.getUserId());
				...
				if (destinationList != null && !destinationList.isEmpty()) {
					for (int i = 0; i < destinationList.size(); i++) {
						...
						newQuestionnairesStepsBoList.get(i).setDestinationStep(
								desId);
	...
	public String deleteFromStepQuestion(Integer formId, Integer questionId,
		...
		try {
			...
			query = session.getNamedQuery("getFormQuestion")
			...
			if (formMappingBo != null) {
				String updateQuery = "update FormMappingBo FMBO set FMBO.sequenceNo=FMBO.sequenceNo-1 where FMBO.formId="
						...
						+ " and FMBO.active=1 and FMBO.sequenceNo >="
			...
			activitydetails = "Question of form step was deleted. (Study ID = "
					+ customStudyId + ")";
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - deleteFromStepQuestion() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public String deleteQuestionnaireStep(Integer stepId,
		...
		try {
			...
			if (studyVersionBo != null) {
				...
				searchQuery = "From QuestionnairesStepsBo QSBO where QSBO.instructionFormId="
						+ stepId
						+ " and QSBO.questionnairesId="
						+ questionnaireId
						+ " and QSBO.stepType='"
						+ stepType
						+ "'";
				questionnairesStepsBo = (QuestionnairesStepsBo) session
				...
				if (questionnairesStepsBo != null) {
					...
					if (questionnairesStepsBo.getStepType().equalsIgnoreCase(
						...
						activity = FdahpStudyDesignerConstants.INSTRUCTION_ACTIVITY
								+ " was deleted.";
						activitydetails = "Instruction step was deleted. (Study ID = "
								+ customStudyId + ")";
						...
						activity = FdahpStudyDesignerConstants.QUESTIONSTEP_ACTIVITY
								+ " was deleted.";
						activitydetails = "Question step was deleted. (Study ID = "
								+ customStudyId + ")";
					} else if (questionnairesStepsBo.getStepType()
							.equalsIgnoreCase(
									FdahpStudyDesignerConstants.FORM_STEP)) {
						activity = FdahpStudyDesignerConstants.FORMSTEP_ACTIVITY
								+ " was deleted.";
						activitydetails = "Form step was deleted .(Study ID = "
								+ customStudyId + ")";
					}
					message = FdahpStudyDesignerConstants.SUCCESS;
				}
				...
				message = deleteQuestionnaireStep(stepId, questionnaireId,
						stepType, customStudyId, sessionObject, session,
						transaction);
			}
			// Reset destination steps in Questionnaire Starts
			searchQuery = "From QuestionnairesStepsBo QSBO where QSBO.questionnairesId="
					+ questionnaireId
					+ " and QSBO.active=1 order by QSBO.sequenceNo ASC";

			questionnaireStepList = session.createQuery(searchQuery).list();
			if (null != questionnaireStepList
					&& !questionnaireStepList.isEmpty()) {
				if (questionnaireStepList.size() == 1) {
					...
					for (i = 0; i < questionnaireStepList.size() - 1; i++) {
						questionnaireStepList.get(i).setDestinationStep(
								questionnaireStepList.get(i + 1).getStepId());
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - deleteQuestionnaireStep() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public String deleteQuestionnaireStep(Integer stepId,
			...
			SessionObject sessionObject, Session session,
		...
		try {
			...
			if (questionnairesStepsBo != null) {
				String updateQuery = "update QuestionnairesStepsBo QSBO set QSBO.sequenceNo=QSBO.sequenceNo-1,QSBO.modifiedBy="
						...
						+ ",QSBO.modifiedOn='"
						+ FdahpStudyDesignerUtil.getCurrentDateTime()
						+ "' where QSBO.questionnairesId="
						+ questionnairesStepsBo.getQuestionnairesId()
						+ " and QSBO.active=1 and QSBO.sequenceNo >="
						+ questionnairesStepsBo.getSequenceNo();
				query = session.createQuery(updateQuery);
				query.executeUpdate();
				...
				if (stepType
					...
					activitydetails = "Instruction Step was deleted.(Study ID ="
							+ customStudyId + ")";
					auditLogDAO
							.saveToAuditLog(session, transaction,
									sessionObject, activity, activitydetails,
									"StudyQuestionnaireDAOImpl - deleteQuestionnaireStep");
					...
					String subQuery = "select FMBO.questionId from FormMappingBo FMBO where FMBO.formId="
							+ stepId;
					...
					String formMappingDelete = "delete FormMappingBo FMBO where FMBO.formId="
							+ stepId;
					query = session.createQuery(formMappingDelete);
					query.executeUpdate();

					String formDelete = "delete FormBo FBO where FBO.formId="
							+ stepId;
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - deleteQuestionnaireStep(session,transction) - ERROR ",
					e);
		}
	...
	public String deleteQuestuionnaireInfo(Integer studyId,
		...
		try {
			...
			if (studyVersionBo != null) {
				...
				// doing the hard delete before study launch
				message = deleteQuestuionnaireInfo(studyId, questionnaireId,
						customStudyId, session, transaction);
			}
			activity = FdahpStudyDesignerConstants.QUESTIONNAIRE_ACTIVITY
					+ " was deleted.";
			activitydetails = FdahpStudyDesignerConstants.QUESTIONNAIRE_ACTIVITY
					+ " was deleted. (Study ID = " + customStudyId + ")";
			auditLogDAO.saveToAuditLog(session, transaction, sessionObject,
					activity, activitydetails,
					"StudyQuestionnaireDAOImpl - deleteQuestuionnaireInfo");

			queryString = "DELETE From NotificationBO where questionnarieId="
					+ questionnaireId + "AND notificationSent=false";
			session.createQuery(queryString).executeUpdate();
			transaction.commit();
		} catch (Exception e) {
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - deleteQuestuionnaireInfo() - Error",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public String deleteQuestuionnaireInfo(Integer studyId,
		...
		try {
			String deleteInsQuery = "delete InstructionsBo IBO where IBO.id IN (select QSBO.instructionFormId from QuestionnairesStepsBo QSBO where QSBO.questionnairesId="
					+ questionnaireId
					+ " and QSBO.active=1 and QSBO.stepType='"
					+ FdahpStudyDesignerConstants.INSTRUCTION_STEP + "')";
			...
			if (query.list() != null && !query.list().isEmpty()) {
				...
				String deleteFormQuery = "delete QuestionsBo QBO where QBO.id IN ("
						+ subQuery + ")";
			...
			String searchQuery = "delete QuestionnairesStepsBo QSBO where QSBO.questionnairesId="
					+ questionnaireId;
			...
			String deleteFreQuery = "delete from questionnaires_frequencies where questionnaires_id="
					+ questionnaireId;
			query = session.createSQLQuery(deleteFreQuery);
			query.executeUpdate();

			String deleteQuery = "delete QuestionnaireBo QBO where QBO.studyId="
					+ studyId + " and QBO.id=" + questionnaireId;
			query = session.createQuery(deleteQuery);
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - deleteQuestuionnaireInfo(session,transction) - ERROR ",
					e);
		}
	...
	public List<HealthKitKeysInfo> getHeanlthKitKeyInfoList() {
		...
		try {
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getQuestionReponseTypeList() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	/**
	 * Instruction step page in questionnaire.Lays down instructions for the user in mobile app.Which contains 
	 * the short title instruction title and text
	 * 
	 * @author BTC
	 * @param Integer
	...
	public InstructionsBo getInstructionsBo(Integer instructionId,
		...
		try {
			session = hibernateTemplate.getSessionFactory().openSession();
			instructionsBo = (InstructionsBo) session.get(InstructionsBo.class,
					instructionId);
			if (instructionsBo != null) {
				if (questionnaireId != null) {
					query = session
							.createQuery("From QuestionnairesStepsBo QSBO where QSBO.instructionFormId="
									+ instructionsBo.getId()
									+ " and QSBO.stepType='"
									+ FdahpStudyDesignerConstants.INSTRUCTION_STEP
					...
					query = session
							...
							.setInteger("instructionFormId",
									instructionsBo.getId())
							.setString(
									"stepType",
									FdahpStudyDesignerConstants.INSTRUCTION_STEP);
				}
				questionnairesStepsBo = (QuestionnairesStepsBo) query
						.uniqueResult();
				...
				if (StringUtils.isNotEmpty(questionnaireShortTitle)) {
					...
					if (shortTitleCount != null
						...
						questionnairesStepsBo
								.setIsShorTitleDuplicate(shortTitleCount
										.intValue());
					else
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getInstructionsBo() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	/**
	 * Get the condition(formula) of question which are created in the response level attributes 
	 * of question in formula based destination enabled section
	 * 
	 * @author BTC
	 * @param session
	 *            {@link Session}
	...
	public List<QuestionConditionBranchBo> getQuestionConditionalBranchingLogic(
		...
		try {
			...
			String searchQuery = "From QuestionConditionBranchBo QCBO where QCBO.questionId="
					+ questionId + " order by QCBO.sequenceNo ASC";
			if (newSession != null) {
				...
				if (questionConditionBranchList != null
					...
					for (QuestionConditionBranchBo questionConditionBranchBo : questionConditionBranchList) {
						if (questionConditionBranchBo.getInputType() != null
								&& (questionConditionBranchBo.getInputType()
										.equalsIgnoreCase("MF") || questionConditionBranchBo
										.getInputType().equalsIgnoreCase("F"))) {
							...
							for (QuestionConditionBranchBo conditionBranchBo : questionConditionBranchList) {
								if (questionConditionBranchBo.getSequenceNo()
										.equals(conditionBranchBo
												.getParentSequenceNo())) {
									conditionBranchList.add(conditionBranchBo);
								}
							}
							questionConditionBranchBo
									.setQuestionConditionBranchBos(conditionBranchList);
							newQuestionConditionBranchList
									.add(questionConditionBranchBo);
						}
					}
				}
			}
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getQuestionConditionalBranchingLogic() - ERROR ",
					e);
		} finally {
			if (null != newSession) {
				newSession.close();
	...
	public QuestionnaireBo getQuestionnaireById(Integer questionnaireId,
		...
		try {
			session = hibernateTemplate.getSessionFactory().openSession();
			questionnaireBo = (QuestionnaireBo) session.get(
					QuestionnaireBo.class, questionnaireId);
			if (null != questionnaireBo) {
				if (StringUtils.isNotEmpty(customStudyId)) {
					...
					if (shortTitleCount != null
							&& shortTitleCount.intValue() > 0)
						questionnaireBo.setShortTitleDuplicate(shortTitleCount
								.intValue());
					else
				...
				String searchQuery = "";
				if (null != questionnaireBo.getFrequency()
						&& !questionnaireBo.getFrequency().isEmpty()) {
					if (questionnaireBo
							.getFrequency()
							.equalsIgnoreCase(
									FdahpStudyDesignerConstants.FREQUENCY_TYPE_MANUALLY_SCHEDULE)) {
						searchQuery = "From QuestionnaireCustomScheduleBo QCSBO where QCSBO.questionnairesId="
								+ questionnaireBo.getId();
						query = session.createQuery(searchQuery);
						List<QuestionnaireCustomScheduleBo> questionnaireCustomScheduleList = query
								.list();
						questionnaireBo
								.setQuestionnaireCustomScheduleBo(questionnaireCustomScheduleList);
					} else {
						searchQuery = "From QuestionnairesFrequenciesBo QFBO where QFBO.questionnairesId="
								+ questionnaireBo.getId();
						query = session.createQuery(searchQuery);
						if (questionnaireBo
							...
							List<QuestionnairesFrequenciesBo> questionnairesFrequenciesList = query
									.list();
							questionnaireBo
									.setQuestionnairesFrequenciesList(questionnairesFrequenciesList);
						} else {
				...
				if (questionnaireBo.getVersion() != null) {
					questionnaireBo.setQuestionnarieVersion(" (V"
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getQuestionnaireById() - Error",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public QuestionnairesStepsBo getQuestionnaireStep(Integer stepId,
		...
		try {
			...
			if (null != questionnairesStepsBo
					&& questionnairesStepsBo.getStepType() != null) {
				if (StringUtils.isNotEmpty(questionnaireShortTitle)) {
					// Duplicate ShortTitle per QuestionnaireStepBo Start
					BigInteger shortTitleCount = (BigInteger) session
							.createSQLQuery(
				...
				if (questionnairesStepsBo.getStepType().equalsIgnoreCase(
					...
					query = session.getNamedQuery("getQuestionStep")
							.setInteger("stepId", stepId);
					...
					if (questionsBo != null && questionsBo.getId() != null) {
						if (StringUtils.isNotEmpty(questionnaireShortTitle)) {
							...
							if (StringUtils.isNotEmpty(questionsBo
									.getStatShortName())) {
								BigInteger quesionStatshortTitleCount = (BigInteger) session
										.createSQLQuery(
								...
								if (quesionStatshortTitleCount != null
										&& quesionStatshortTitleCount
												.intValue() > 0)
									questionsBo
											.setIsStatShortNameDuplicate(quesionStatshortTitleCount
													.intValue());
								else
						...
						logger.info("StudyQuestionnaireDAOImpl - getQuestionnaireStep() - questionsResponseTypeId:"
								+ questionsBo.getId());
						query = session.getNamedQuery("getQuestionResponse")
								.setInteger("questionsResponseTypeId",
										questionsBo.getId());
						query.setMaxResults(1);
						questionReponseTypeBo = (QuestionReponseTypeBo) query
								.uniqueResult();
						if (questionReponseTypeBo != null
								&& questionReponseTypeBo.getStyle() != null
								&& StringUtils.isNotEmpty(questionReponseTypeBo
										.getStyle())) {
							questionReponseTypeBo
									.setStyle(questionReponseTypeBo.getStyle());
							if ((FdahpStudyDesignerConstants.DATE)
									.equalsIgnoreCase(questionReponseTypeBo
											.getStyle())) {
								if (questionReponseTypeBo.getMinDate() != null
										&& StringUtils
												.isNotEmpty(questionReponseTypeBo
														.getMinDate())) {
									questionReponseTypeBo
											.setMinDate(FdahpStudyDesignerUtil.getFormattedDate(
													questionReponseTypeBo
															.getMinDate(),
								...
								if (questionReponseTypeBo.getMaxDate() != null
										&& StringUtils
												.isNotEmpty(questionReponseTypeBo
														.getMaxDate())) {
									questionReponseTypeBo
											.setMaxDate(FdahpStudyDesignerUtil.getFormattedDate(
													questionReponseTypeBo
															.getMaxDate(),
													FdahpStudyDesignerConstants.DB_SDF_DATE,
													FdahpStudyDesignerConstants.UI_SDF_DATE));
								...
								if (questionReponseTypeBo.getDefaultDate() != null
										&& StringUtils
												.isNotEmpty(questionReponseTypeBo
														.getDefaultDate())) {
									questionReponseTypeBo
											.setDefaultDate(FdahpStudyDesignerUtil.getFormattedDate(
													questionReponseTypeBo
															.getDefaultDate(),
													FdahpStudyDesignerConstants.DB_SDF_DATE,
													FdahpStudyDesignerConstants.UI_SDF_DATE));
						...
						if (questionReponseTypeBo != null
							...
							questionnairesStepsBo
									.setQuestionConditionBranchBoList(questionConditionBranchList);
						}
						questionnairesStepsBo
								.setQuestionReponseTypeBo(questionReponseTypeBo);

						List<QuestionResponseSubTypeBo> questionResponseSubTypeList = null;
						query = session.getNamedQuery("getQuestionSubResponse")
								.setInteger("responseTypeId",
										questionsBo.getId());
						questionResponseSubTypeList = query.list();
						// appending the current date time to the image url
						if (null != questionResponseSubTypeList
								&& !questionResponseSubTypeList.isEmpty()) {
							for (QuestionResponseSubTypeBo s : questionResponseSubTypeList) {
								if (FdahpStudyDesignerUtil.isNotEmpty(s
										.getImage())) {
									if (s.getImage().contains("?v=")) {
										String imagePathArr[] = s.getImage()
												.split("\\?");
										s.setImage(imagePathArr[0] + "?v="
												+ new Date().getTime());
									} else {
										s.setImage(s.getImage() + "?v="
												+ new Date().getTime());
									}
								}
								if (FdahpStudyDesignerUtil.isNotEmpty(s
									...
									if (s.getSelectedImage().contains("?v=")) {
										String imagePathArr[] = s
												.getSelectedImage()
												.split("\\?");
										s.setSelectedImage(imagePathArr[0]
												+ "?v=" + new Date().getTime());
									} else {
										s.setSelectedImage(s.getSelectedImage()
												+ "?v=" + new Date().getTime());
									}
								}
							}
					...
					Iterator iterator = session.createSQLQuery(fromQuery)
							.list().iterator();
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getQuestionnaireStep() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public SortedMap<Integer, QuestionnaireStepBean> getQuestionnaireStepList(
		...
		try {
			...
			for (QuestionnairesStepsBo questionaireSteps : questionnairesStepsList) {
				destinationText.put(questionaireSteps.getStepId(),
						questionaireSteps.getSequenceNo() + ":"
								+ questionaireSteps.getStepShortTitle());
				switch (questionaireSteps.getStepType()) {
				case FdahpStudyDesignerConstants.INSTRUCTION_STEP:
					instructionIdList.add(questionaireSteps
							.getInstructionFormId());
					sequenceNoMap
							.put(String.valueOf(questionaireSteps
									.getInstructionFormId())
									+ FdahpStudyDesignerConstants.INSTRUCTION_STEP,
									questionaireSteps.getSequenceNo());
					destinationMap
				...
				case FdahpStudyDesignerConstants.QUESTION_STEP:
					questionIdList
							.add(questionaireSteps.getInstructionFormId());
					sequenceNoMap
							.put(String.valueOf(questionaireSteps
									.getInstructionFormId())
									+ FdahpStudyDesignerConstants.QUESTION_STEP,
				...
				case FdahpStudyDesignerConstants.FORM_STEP:
					...
					sequenceNoMap.put(
							String.valueOf(questionaireSteps
									.getInstructionFormId())
									+ FdahpStudyDesignerConstants.FORM_STEP,
							questionaireSteps.getSequenceNo());
					destinationMap.put(
							String.valueOf(questionaireSteps
									...
									+ FdahpStudyDesignerConstants.FORM_STEP,
							questionaireSteps.getDestinationStep());
					formStatusMap.put(questionaireSteps.getInstructionFormId(),
							questionaireSteps.getStatus());
					break;
				default:
					break;
				}
			...
			if (!instructionIdList.isEmpty()) {
				...
				query = session
						.createQuery(" from InstructionsBo IBO where IBO.active=1 and IBO.id in ("
				...
				if (instructionsList != null && !instructionsList.isEmpty()) {
					for (InstructionsBo instructionsBo : instructionsList) {
						...
						questionnaireStepBean
								.setStepType(FdahpStudyDesignerConstants.INSTRUCTION_STEP);
						questionnaireStepBean
								.setSequenceNo(sequenceNoMap.get(instructionsBo
										.getId()
										+ FdahpStudyDesignerConstants.INSTRUCTION_STEP));
						questionnaireStepBean.setTitle(instructionsBo
								.getInstructionTitle());
						questionnaireStepBean.setStatus(instructionsBo
								.getStatus());
						questionnaireStepBean
								.setDestinationStep(destinationMap
										.get(instructionsBo.getId()
												+ FdahpStudyDesignerConstants.INSTRUCTION_STEP));
						questionnaireStepBean
								.setDestinationText(destinationText.get(destinationMap
										.get(instructionsBo.getId()
												+ FdahpStudyDesignerConstants.INSTRUCTION_STEP)));
						qTreeMap.put(
								sequenceNoMap
			...
			if (!questionIdList.isEmpty()) {
				...
				query = session
						.createQuery(" from QuestionsBo QBO where QBO.active=1 and QBO.id in ("
				...
				if (questionsList != null && !questionsList.isEmpty()) {
					for (QuestionsBo questionsBo : questionsList) {
						...
						questionnaireStepBean
								.setStepType(FdahpStudyDesignerConstants.QUESTION_STEP);
						questionnaireStepBean
								.setSequenceNo(sequenceNoMap.get(questionsBo
										.getId()
										+ FdahpStudyDesignerConstants.QUESTION_STEP));
						questionnaireStepBean.setTitle(questionsBo
								.getQuestion());
						questionnaireStepBean.setResponseType(questionsBo
								.getResponseType());
						questionnaireStepBean.setLineChart(questionsBo
								.getAddLineChart());
						questionnaireStepBean.setStatData(questionsBo
								.getUseStasticData());
						questionnaireStepBean
								.setStatus(questionsBo.getStatus());
						questionnaireStepBean
								.setDestinationStep(destinationMap
										.get(questionsBo.getId()
												+ FdahpStudyDesignerConstants.QUESTION_STEP));
						questionnaireStepBean.setUseAnchorDate(questionsBo
								.getUseAnchorDate());
						questionnaireStepBean
								.setDestinationText(destinationText.get(destinationMap
										.get(questionsBo.getId()
												+ FdahpStudyDesignerConstants.QUESTION_STEP)));
						qTreeMap.put(sequenceNoMap.get(questionsBo.getId()
								+ FdahpStudyDesignerConstants.QUESTION_STEP),
								questionnaireStepBean);
					}
				}
				...
				for (int i = 0; i < formIdList.size(); i++) {
					...
					for (int j = 0; j < result.size(); j++) {
						...
						if (formIdList.get(i).equals(formId)) {
							...
							questionnaireStepBean.setStatus(status);
							questionnaireStepBean
									.setUseAnchorDate(useAnchorDate);
							formQuestionMap.put(sequenceNo,
									questionnaireStepBean);
						}
					}
					...
					fQuestionnaireStepBean
							.setStepType(FdahpStudyDesignerConstants.FORM_STEP);
					fQuestionnaireStepBean.setSequenceNo(sequenceNoMap
							.get(formIdList.get(i)
									+ FdahpStudyDesignerConstants.FORM_STEP));
					fQuestionnaireStepBean.setFromMap(formQuestionMap);
					fQuestionnaireStepBean.setStatus(formStatusMap
							.get(formIdList.get(i)));
					fQuestionnaireStepBean.setDestinationStep(destinationMap
							.get(formIdList.get(i)
									+ FdahpStudyDesignerConstants.FORM_STEP));
					fQuestionnaireStepBean.setDestinationText(destinationText
							.get(destinationMap.get(formIdList.get(i)
									+ FdahpStudyDesignerConstants.FORM_STEP)));
					qTreeMap.put(
							sequenceNoMap.get(formIdList.get(i)
									+ FdahpStudyDesignerConstants.FORM_STEP),
							fQuestionnaireStepBean);
				}
			}
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getQuestionnaireStepList() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public QuestionsBo getQuestionsById(Integer questionId,
		...
		try {
			session = hibernateTemplate.getSessionFactory().openSession();
			questionsBo = (QuestionsBo) session.get(QuestionsBo.class,
					questionId);
			if (questionsBo != null) {
				try {
					if (StringUtils.isNotEmpty(questionnaireShortTitle)) {
						// Duplicate ShortTitle per questionsBo Start
						BigInteger quesionshortTitleCount = (BigInteger) session
								.createSQLQuery(
										"select count(*) From questions QBO,form_mapping f,questionnaires_steps QSBO,questionnaires Q where QBO.id=f.question_id and f.form_id=QSBO.instruction_form_id and QSBO.questionnaires_id=Q.id and Q.short_title='"
												...
												+ customStudyId
												+ "' and QSBO.step_type='Form' and QBO.short_title='"
												+ questionsBo.getShortTitle()
												+ "' and QBO.active=1")
								.uniqueResult();
						if (quesionshortTitleCount != null
								&& quesionshortTitleCount.intValue() > 0)
							questionsBo
									.setIsShorTitleDuplicate(quesionshortTitleCount
											.intValue());
						else
					...
					logger.error(
							"StudyQuestionnaireDAOImpl - getQuestionsById() - SUB  ERROR ",
							e);
				}
				QuestionReponseTypeBo questionReponseTypeBo = null;
				logger.info("StudyQuestionnaireDAOImpl - getQuestionnaireStep() - questionsResponseTypeId:"
				...
				if (questionReponseTypeBo != null
						...
						&& StringUtils.isNotEmpty(questionReponseTypeBo
								.getStyle())) {
					questionReponseTypeBo.setStyle(questionReponseTypeBo
							.getStyle());
				...
				if (null != questionResponseSubTypeList
					...
					for (QuestionResponseSubTypeBo s : questionResponseSubTypeList) {
						...
						if (FdahpStudyDesignerUtil.isNotEmpty(s
								.getSelectedImage())) {
							if (s.getSelectedImage().contains("?v=")) {
								String imagePathArr[] = s.getSelectedImage()
										.split("\\?");
								s.setSelectedImage(imagePathArr[0] + "?v="
										+ new Date().getTime());
							} else {
								s.setSelectedImage(s.getSelectedImage() + "?v="
										+ new Date().getTime());
							}
						}
					}
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getQuestionsById() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	/**
	 * Return the QuestionReponsetype based on the response type id which already exists for question.These values are listed in
	...
	public QuestionReponseTypeBo getQuestionsResponseTypeBo(
		...
		try {
			if (questionsResponseTypeBo != null && session != null) {
				if (questionsResponseTypeBo.getResponseTypeId() != null) {
					addOrUpdateQuestionsResponseTypeBo = (QuestionReponseTypeBo) session
							.get(QuestionReponseTypeBo.class,
									questionsResponseTypeBo.getResponseTypeId());
				} else {
				...
				if (questionsResponseTypeBo.getQuestionsResponseTypeId() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setQuestionsResponseTypeId(questionsResponseTypeBo
									.getQuestionsResponseTypeId());
				}
				if (questionsResponseTypeBo.getMinValue() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setMinValue(questionsResponseTypeBo.getMinValue());
				}
				if (questionsResponseTypeBo.getMaxValue() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setMaxValue(questionsResponseTypeBo.getMaxValue());
				}
				if (questionsResponseTypeBo.getDefaultValue() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setDefaultValue(questionsResponseTypeBo
									.getDefaultValue());
				}
				if (questionsResponseTypeBo.getStep() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setStep(questionsResponseTypeBo.getStep());
				}
				if (questionsResponseTypeBo.getVertical() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setVertical(questionsResponseTypeBo.getVertical());
				}
				addOrUpdateQuestionsResponseTypeBo
						.setMinDescription(questionsResponseTypeBo
								.getMinDescription());
				addOrUpdateQuestionsResponseTypeBo
						.setMaxDescription(questionsResponseTypeBo
								.getMaxDescription());
				if (questionsResponseTypeBo.getMaxFractionDigits() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setMaxFractionDigits(questionsResponseTypeBo
									.getMaxFractionDigits());
				}
				if (questionsResponseTypeBo.getTextChoices() != null
						&& StringUtils.isNotEmpty(questionsResponseTypeBo
								.getTextChoices())) {
					addOrUpdateQuestionsResponseTypeBo
							.setTextChoices(questionsResponseTypeBo
									.getTextChoices());
				}
				if (questionsResponseTypeBo.getSelectionStyle() != null
						&& StringUtils.isNotEmpty(questionsResponseTypeBo
								.getSelectionStyle())) {
					addOrUpdateQuestionsResponseTypeBo
							.setSelectionStyle(questionsResponseTypeBo
									.getSelectionStyle());
				}
				if (questionsResponseTypeBo.getImageSize() != null
						&& StringUtils.isNotEmpty(questionsResponseTypeBo
								.getImageSize())) {
					addOrUpdateQuestionsResponseTypeBo
							.setImageSize(questionsResponseTypeBo
									.getImageSize());
				}
				if (questionsResponseTypeBo.getStyle() != null
						&& StringUtils.isNotEmpty(questionsResponseTypeBo
								.getStyle())) {
					addOrUpdateQuestionsResponseTypeBo
							.setStyle(questionsResponseTypeBo.getStyle());
					if ((FdahpStudyDesignerConstants.DATE)
							.equalsIgnoreCase(questionsResponseTypeBo
									.getStyle())) {
						if (questionsResponseTypeBo.getMinDate() != null
								&& StringUtils
										.isNotEmpty(questionsResponseTypeBo
												.getMinDate())) {
							addOrUpdateQuestionsResponseTypeBo
									.setMinDate(FdahpStudyDesignerUtil.getFormattedDate(
				...
				addOrUpdateQuestionsResponseTypeBo
						.setPlaceholder(questionsResponseTypeBo
								.getPlaceholder());
				addOrUpdateQuestionsResponseTypeBo
						.setUnit(questionsResponseTypeBo.getUnit());
				addOrUpdateQuestionsResponseTypeBo
						.setMaxLength(questionsResponseTypeBo.getMaxLength());
				if (questionsResponseTypeBo.getValidationRegex() != null
						&& StringUtils.isNotEmpty(questionsResponseTypeBo
								.getValidationRegex())) {
					addOrUpdateQuestionsResponseTypeBo
							.setValidationRegex(questionsResponseTypeBo
									.getValidationRegex());
				}
				addOrUpdateQuestionsResponseTypeBo
						.setInvalidMessage(questionsResponseTypeBo
								.getInvalidMessage());
				if (questionsResponseTypeBo.getMultipleLines() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setMultipleLines(questionsResponseTypeBo
									.getMultipleLines());
				}
				if (questionsResponseTypeBo.getMeasurementSystem() != null
					...
					addOrUpdateQuestionsResponseTypeBo
							.setMeasurementSystem(questionsResponseTypeBo
									.getMeasurementSystem());
				}
				if (questionsResponseTypeBo.getUseCurrentLocation() != null) {
					addOrUpdateQuestionsResponseTypeBo
							.setUseCurrentLocation(questionsResponseTypeBo
									.getUseCurrentLocation());
				}
				addOrUpdateQuestionsResponseTypeBo
						.setValidationCondition(questionsResponseTypeBo
								.getValidationCondition());
				addOrUpdateQuestionsResponseTypeBo
						.setValidationCharacters(questionsResponseTypeBo
								.getValidationCharacters());
				addOrUpdateQuestionsResponseTypeBo
						.setValidationExceptText(questionsResponseTypeBo
								.getValidationExceptText());

				addOrUpdateQuestionsResponseTypeBo
						.setValidationRegex(FdahpStudyDesignerUtil
								.getRegExpression(questionsResponseTypeBo
										.getValidationCondition(),
										questionsResponseTypeBo
												.getValidationCharacters(),
										questionsResponseTypeBo
				...
				if (questionsResponseTypeBo.getMinImageFile() != null) {
					if (questionsResponseTypeBo.getMinImage() != null
							&& StringUtils.isNotEmpty(questionsResponseTypeBo
									.getMinImage())) {
						addOrUpdateQuestionsResponseTypeBo
								.setMinImage(questionsResponseTypeBo
										.getMinImage());
					} else {
						if (questionsResponseTypeBo.getMinImageFile()
								.getOriginalFilename() != null
								&& StringUtils
										.isNotEmpty(questionsResponseTypeBo
												.getMinImageFile()
												.getOriginalFilename())) {
							fileName = FdahpStudyDesignerUtil
									.getStandardFileName(
											FdahpStudyDesignerConstants.QUESTION_STEP_IMAGE + 0,
											questionsResponseTypeBo
													.getMinImageFile()
													.getOriginalFilename(),
											String.valueOf(questionsResponseTypeBo
													.getQuestionsResponseTypeId()));
							String imagePath = FdahpStudyDesignerUtil
									.uploadImageFile(
											questionsResponseTypeBo
													.getMinImageFile(),
											fileName,
					...
					if (StringUtils.isEmpty(questionsResponseTypeBo
							.getMinImage())) {
				...
				if (questionsResponseTypeBo.getMaxImageFile() != null) {
					if (questionsResponseTypeBo.getMaxImage() != null
							&& StringUtils.isNotEmpty(questionsResponseTypeBo
									.getMaxImage())) {
						addOrUpdateQuestionsResponseTypeBo
								.setMaxImage(questionsResponseTypeBo
										.getMaxImage());
					} else {
						if (questionsResponseTypeBo.getMaxImageFile()
								.getOriginalFilename() != null
								&& StringUtils
										.isNotEmpty(questionsResponseTypeBo
												.getMaxImageFile()
				...
				if (questionsResponseTypeBo.getDefaultTime() != null
						&& StringUtils.isNotEmpty(questionsResponseTypeBo
								.getDefaultTime())) {
					addOrUpdateQuestionsResponseTypeBo
							.setDefaultTime(questionsResponseTypeBo
									.getDefaultTime());
				}
				if (questionsResponseTypeBo.getFormulaBasedLogic() != null
						&& StringUtils.isNotEmpty(questionsResponseTypeBo
								.getFormulaBasedLogic())) {
					addOrUpdateQuestionsResponseTypeBo
							.setFormulaBasedLogic(questionsResponseTypeBo
									.getFormulaBasedLogic());
				}
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getQuestionsResponseTypeBo() - Error",
					e);
		}
	...
	public List<QuestionnaireBo> getStudyQuestionnairesByStudyId(
		...
		try {
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - getStudyQuestionnairesByStudyId() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public Boolean isQuestionnairesCompleted(Integer studyId) {
		...
		try {
			...
			BigDecimal count = (BigDecimal) session.createSQLQuery(searchQuery)
					.uniqueResult();
			if (count != null && count.intValue() > 0) {
				isExists = false;
			}
	...
	public String reOrderFormStepQuestions(Integer formId, int oldOrderNumber,
		...
		try {
			...
			transaction = session.beginTransaction();
			String updateQuery = "";
			query = session.getNamedQuery("getFromByIdAndSequenceNo")
					.setInteger("formId", formId)
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - reOrderFormStepQuestions() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public String reOrderQuestionnaireSteps(Integer questionnaireId,
		...
		try {
			...
			String updateQuery = "";
			query = session
					.createQuery("From QuestionnairesStepsBo QSBO where QSBO.questionnairesId="
							+ questionnaireId
							+ " and QSBO.sequenceNo ="
			...
			if (questionnairesStepsBo != null) {
				...
				if (oldOrderNumber < newOrderNumber) {
					...
					if (count > 0) {
						query = session
								.createQuery("update QuestionnairesStepsBo q set q.sequenceNo="
										+ newOrderNumber
										+ " where q.stepId="
										+ questionnairesStepsBo.getStepId()
										+ " and q.active=1");
				...
				if (message
					...
					String searchQuery = "From QuestionnairesStepsBo QSBO where QSBO.questionnairesId="
							+ questionnaireId
							+ "and QSBO.active=1 order by QSBO.sequenceNo ASC";
					questionnaireStepList = session.createQuery(searchQuery)
							.list();
					if (null != questionnaireStepList
							&& !questionnaireStepList.isEmpty()) {
						if (questionnaireStepList.size() == 1) {
							...
							for (i = 0; i < questionnaireStepList.size() - 1; i++) {
								questionnaireStepList
										...
										.setDestinationStep(
												questionnaireStepList
														.get(i + 1).getStepId());
								questionnaireStepList.get(i).setSequenceNo(
										i + 1);
								session.update(questionnaireStepList.get(i));
							}
							questionnaireStepList.get(i).setDestinationStep(0);
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - reOrderQuestionnaireSteps() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
		...
		try {
			...
			if (questionnairesStepsBo != null) {
				if (questionnairesStepsBo.getStepId() != null) {
					addOrUpdateQuestionnairesStepsBo = (QuestionnairesStepsBo) session
							.get(QuestionnairesStepsBo.class,
									questionnairesStepsBo.getStepId());
				} else {
				...
				if (questionnairesStepsBo.getType() != null) {
					if (questionnairesStepsBo.getType().equalsIgnoreCase(
						...
						query = session
								.createSQLQuery("update questionnaires q set q.status=0 where q.id="
										+ addOrUpdateQuestionnairesStepsBo
												.getQuestionnairesId());
						query.executeUpdate();
					} else if (questionnairesStepsBo
				...
				if (addOrUpdateQuestionnairesStepsBo.getQuestionnairesId() != null
					...
					formBo.setCreatedOn(addOrUpdateQuestionnairesStepsBo
							.getCreatedOn());
					formBo.setCreatedBy(addOrUpdateQuestionnairesStepsBo
							.getCreatedBy());
					session.saveOrUpdate(formBo);
					addOrUpdateQuestionnairesStepsBo
							.setQuestionnairesId(addOrUpdateQuestionnairesStepsBo
									.getQuestionnairesId());
					addOrUpdateQuestionnairesStepsBo
							.setInstructionFormId(formBo.getFormId());
					addOrUpdateQuestionnairesStepsBo
							.setStepType(FdahpStudyDesignerConstants.FORM_STEP);
					QuestionnairesStepsBo existedQuestionnairesStepsBo = null;
					query = session.getNamedQuery(
							"getQuestionnaireStepSequenceNo").setInteger(
							"questionnairesId",
							addOrUpdateQuestionnairesStepsBo
									.getQuestionnairesId());
					query.setMaxResults(1);
					existedQuestionnairesStepsBo = (QuestionnairesStepsBo) query
							.uniqueResult();
				...
				if (addOrUpdateQuestionnairesStepsBo != null && count > 0) {
					String updateQuery = "update QuestionnairesStepsBo QSBO set QSBO.destinationStep="
							+ addOrUpdateQuestionnairesStepsBo.getStepId()
							+ " where "
			...
			auditLogDAO
					.saveToAuditLog(session, transaction, sesObj, activity,
							activitydetails,
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - saveOrUpdateFromQuestionnaireStep() - Error",
					e);
		} finally {
			if (session != null) {
				session.close();
		...
		try {
			...
			if (instructionsBo != null && instructionsBo.getId() != null
				...
				if (instructionsBo.getQuestionnairesStepsBo().getStepId() != null) {
					questionnairesStepsBo = (QuestionnairesStepsBo) session
							.get(QuestionnairesStepsBo.class, instructionsBo
				...
				questionnairesStepsBo.setQuestionnairesId(instructionsBo
						.getQuestionnaireId());
				questionnairesStepsBo.setInstructionFormId(instructionsBo
						.getId());
				questionnairesStepsBo
						.setStepType(FdahpStudyDesignerConstants.INSTRUCTION_STEP);
				if (instructionsBo.getQuestionnairesStepsBo()
						.getStepShortTitle() != null
						&& !instructionsBo.getQuestionnairesStepsBo()
								.getStepShortTitle().isEmpty()) {
					questionnairesStepsBo.setStepShortTitle(instructionsBo
							.getQuestionnairesStepsBo().getStepShortTitle());
				}
				if (instructionsBo.getQuestionnairesStepsBo().getSkiappable() != null
						&& !instructionsBo.getQuestionnairesStepsBo()
								.getSkiappable().isEmpty()) {
					questionnairesStepsBo.setSkiappable(instructionsBo
							.getQuestionnairesStepsBo().getSkiappable());
				}
				if (instructionsBo.getQuestionnairesStepsBo().getRepeatable() != null
						&& !instructionsBo.getQuestionnairesStepsBo()
								.getRepeatable().isEmpty()) {
					questionnairesStepsBo.setRepeatable(instructionsBo
							.getQuestionnairesStepsBo().getRepeatable());
				}
				if (instructionsBo.getQuestionnairesStepsBo()
						.getRepeatableText() != null
						&& !instructionsBo.getQuestionnairesStepsBo()
								.getRepeatableText().isEmpty()) {
					questionnairesStepsBo.setRepeatableText(instructionsBo
							.getQuestionnairesStepsBo().getRepeatableText());
				}
				if (instructionsBo.getQuestionnairesStepsBo()
						.getDestinationStep() != null) {
					questionnairesStepsBo.setDestinationStep(instructionsBo
							.getQuestionnairesStepsBo().getDestinationStep());
				}
				if (instructionsBo.getQuestionnairesStepsBo().getCreatedOn() != null) {
					questionnairesStepsBo.setCreatedOn(instructionsBo
							.getQuestionnairesStepsBo().getCreatedOn());
				}
				if (instructionsBo.getQuestionnairesStepsBo().getCreatedBy() != null) {
					questionnairesStepsBo.setCreatedBy(instructionsBo
							.getQuestionnairesStepsBo().getCreatedBy());
				}
				if (instructionsBo.getQuestionnairesStepsBo().getModifiedOn() != null) {
					questionnairesStepsBo.setModifiedOn(instructionsBo
							.getQuestionnairesStepsBo().getModifiedOn());
				}
				if (instructionsBo.getQuestionnairesStepsBo().getModifiedBy() != null) {
					questionnairesStepsBo.setModifiedBy(instructionsBo
							.getQuestionnairesStepsBo().getModifiedBy());
				}
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - saveOrUpdateInstructionsBo() - Error",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	/**
	 * Question of a form step contains the two attributes .Question-level attributes – these are the same
	 * set of attributes as that for question step with the exception of the skippable property and branching logic based on participant
		...
		try {
			...
			if (questionsBo != null && questionsBo.getId() != null
				...
				if (addQuestionReponseTypeBo != null) {
					if (addQuestionReponseTypeBo.getQuestionsResponseTypeId() == null) {
						addQuestionReponseTypeBo
								.setQuestionsResponseTypeId(questionsBo.getId());
					}
				...
				if (questionsBo.getQuestionResponseSubTypeList() != null
					...
					if (questionsBo.getResponseType() == 4
						...
						for (QuestionResponseSubTypeBo questionResponseSubTypeBo : questionsBo
							...
							if ((questionResponseSubTypeBo.getText() != null && !questionResponseSubTypeBo
									.getText().isEmpty())
									&& (questionResponseSubTypeBo.getValue() != null && !questionResponseSubTypeBo
											.getValue().isEmpty())) {
								String fileName;
								if (questionResponseSubTypeBo.getImageFile() != null) {
									if (questionResponseSubTypeBo.getImage() != null
											&& !questionResponseSubTypeBo
													.getImage().isEmpty()) {
										questionResponseSubTypeBo
												.setImage(questionResponseSubTypeBo
														.getImage());
									} else {
										fileName = FdahpStudyDesignerUtil
												.getStandardFileName(
														FdahpStudyDesignerConstants.FORM_STEP_IMAGE
																+ i,
														questionResponseSubTypeBo
																.getImageFile()
																.getOriginalFilename(),
														String.valueOf(questionsBo
																.getId()));
										String imagePath = FdahpStudyDesignerUtil
												.uploadImageFile(
														questionResponseSubTypeBo
								...
								if (questionResponseSubTypeBo
									...
									if (questionResponseSubTypeBo
											...
											&& !questionResponseSubTypeBo
													...
													.isEmpty()) {
										questionResponseSubTypeBo
												.setSelectedImage(questionResponseSubTypeBo
														.getSelectedImage());
									} else {
										fileName = FdahpStudyDesignerUtil
												.getStandardFileName(
														FdahpStudyDesignerConstants.FORM_STEP_SELECTEDIMAGE
																+ i,
														questionResponseSubTypeBo
																.getSelectImageFile()
																.getOriginalFilename(),
								...
								questionResponseSubTypeBo
										.setResponseTypeId(questionsBo.getId());
				...
				if (!questionsBo.getStatus()) {
					...
					QuestionnairesStepsBo questionnairesStepsBo = (QuestionnairesStepsBo) query
							.uniqueResult();
					if (questionnairesStepsBo != null
				...
				query = session.getNamedQuery("getFormMappingBO").setInteger(
						"questionId", questionsBo.getId());
				FormMappingBo formMappingBo = (FormMappingBo) query
						.uniqueResult();
				if (formMappingBo == null) {
					...
					int sequenceNo = 0;
					query = session
							.createQuery("From FormMappingBo FMBO where FMBO.formId="
									+ questionsBo.getFromId()
									+ " and FMBO.active=1 order by FMBO.sequenceNo DESC");
					query.setMaxResults(1);
					FormMappingBo existedFormMappingBo = (FormMappingBo) query
							.uniqueResult();
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - saveOrUpdateQuestion() - Error",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public QuestionnaireBo saveORUpdateQuestionnaire(
		...
		try {
			...
			if (questionnaireBo.getType().equalsIgnoreCase(
				...
				if (questionnaireBo != null && questionnaireBo.getId() != null) {
					if (questionnaireBo.getQuestionnairesFrequenciesList() != null
						...
						query = session
								.createSQLQuery(
										"CALL deleteQuestionnaireFrequencies(:questionnaireId)")
								.setInteger("questionnaireId",
										questionnaireBo.getId());
						query.executeUpdate();
						for (QuestionnairesFrequenciesBo questionnairesFrequenciesBo : questionnaireBo
							...
							if (questionnairesFrequenciesBo.getFrequencyTime() != null) {
								...
								if (questionnairesFrequenciesBo
										.getQuestionnairesId() == null) {
									questionnairesFrequenciesBo.setId(null);
									questionnairesFrequenciesBo
											.setQuestionnairesId(questionnaireBo
													.getId());
								}
			...
			if (questionnaireBo.getFrequency() != null
				...
				if (questionnaireBo.getCurrentFrequency() != null
					...
					updateLineChartSchedule(questionnaireBo.getId(),
							questionnaireBo.getCurrentFrequency(),
					...
					&& !questionnaireBo.getFrequency().equalsIgnoreCase(
							questionnaireBo.getPreviousFrequency())) {
				updateLineChartSchedule(questionnaireBo.getId(),
						questionnaireBo.getFrequency(), sessionObject, session,
						transaction, customStudyId);
			}

			activity = "Content saved for questionnaire.";
			activitydetails = "Content saved for questionnaire. (Study ID = "
					+ customStudyId + ").";
			auditLogDAO.saveToAuditLog(session, transaction, sessionObject,
					activity, activitydetails,
			...
			if (questionnaireBo != null && questionnaireBo.getStatus()) {
				...
				if (studyBo != null) {
					...
					queryString = "From NotificationBO where questionnarieId="
							+ questionnaireBo.getId() + "and studyId="
							+ questionnaireBo.getStudyId();
					notificationBO = (NotificationBO) session
							.createQuery(queryString).setMaxResults(1)
							.uniqueResult();
					if (notificationBO == null) {
						...
						notificationBO.setQuestionnarieId(questionnaireBo
								.getId());
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - saveORUpdateQuestionnaire() - Error",
					e);
		} finally {
			if (session != null) {
				session.close();
		...
		try {
			...
			if (questionnairesStepsBo != null) {
				...
				if (questionnairesStepsBo.getQuestionsBo() != null) {
					...
					if (questionsBo != null
						...
						if (questionResponseTypeBo != null) {
							if (questionResponseTypeBo
									.getQuestionsResponseTypeId() == null) {
								questionResponseTypeBo
										.setQuestionsResponseTypeId(questionsBo
												.getId());
							}
							session.saveOrUpdate(questionResponseTypeBo);
						}
						addOrUpdateQuestionnairesStepsBo
								.setQuestionReponseTypeBo(questionResponseTypeBo);
						if (questionnairesStepsBo
							...
							if (questionnairesStepsBo.getQuestionsBo()
								...
								for (QuestionResponseSubTypeBo questionResponseSubTypeBo : questionnairesStepsBo
									...
									if ((questionResponseSubTypeBo.getText() != null && !questionResponseSubTypeBo
											.getText().isEmpty())
											&& (questionResponseSubTypeBo
													.getValue() != null && !questionResponseSubTypeBo
													.getValue().isEmpty())) {
										String fileName;
										if (questionResponseSubTypeBo
												.getImageFile() != null) {
											if (questionResponseSubTypeBo
												...
												fileName = FdahpStudyDesignerUtil
														.getStandardFileName(
																FdahpStudyDesignerConstants.QUESTION_STEP_IMAGE
																		+ j,
																questionResponseSubTypeBo
																		.getImageFile()
																		.getOriginalFilename(),
																String.valueOf(questionnairesStepsBo
																		.getQuestionsBo()
																		.getId()));
												String imagePath = FdahpStudyDesignerUtil
														.uploadImageFile(
										...
										if (questionResponseSubTypeBo
											...
											if (questionResponseSubTypeBo
												...
												fileName = FdahpStudyDesignerUtil
														.getStandardFileName(
																FdahpStudyDesignerConstants.QUESTION_STEP_SELECTEDIMAGE
																		+ j,
																questionResponseSubTypeBo
																		.getSelectImageFile()
																		.getOriginalFilename(),
					...
					if (questionsBo != null) {
						addOrUpdateQuestionnairesStepsBo
								.setInstructionFormId(questionsBo.getId());
					}
					...
					if (addOrUpdateQuestionnairesStepsBo.getQuestionnairesId() != null
						...
						existedQuestionnairesStepsBo = (QuestionnairesStepsBo) query
								.uniqueResult();
						if (existedQuestionnairesStepsBo != null) {
							count = existedQuestionnairesStepsBo
									.getSequenceNo() + 1;
						} else {
							count = count + 1;
						}
			...
			transaction.rollback();
			logger.error(
					"StudyQuestionnaireDAOImpl - saveOrUpdateQuestionStep() - Error",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	/**
	 * Update the questions status to draft when changing the frequency of questionnaire if the line chart is enabled for that question
	...
	public String updateLineChartSchedule(Integer questionnaireId,
			...
			Transaction transaction, String customStudyId) {
		...
		try {
			...
			String searchQuery = "update questions QBO,questionnaires_steps QSBO set QBO.status=0,QBO.modified_by="
					...
					+ ",QBO.modified_on='"
					+ FdahpStudyDesignerUtil.getCurrentDateTime()
					+ "' where QBO.id=QSBO.instruction_form_id and QSBO.questionnaires_id="
					+ questionnaireId
			...
			String formQuery = "update questionnaires_steps qs,form_mapping f, questions QBO  set qs.status=0,qs.modified_by="
					...
					+ "and QBO.line_chart_timerange not in ('"
					+ timeRange
					+ "') and qs.questionnaires_id="
					+ questionnaireId
					+ " and qs.active=1";
			if (newSession != null) {
			...
			activity = FdahpStudyDesignerConstants.FORMSTEP_ACTIVITY
					+ " saved.";
			activitydetails = "Content saved for Form Step. (Study ID = "
					+ customStudyId + ")";
			auditLogDAO.saveToAuditLog(session, transaction, sessionObject,
					activity, activitydetails,
			...
			activity = FdahpStudyDesignerConstants.QUESTIONSTEP_ACTIVITY
					+ " saved.";
			activitydetails = "Content saved for Question Step. (Study ID = "
					+ customStudyId + ")";
			auditLogDAO.saveToAuditLog(session, transaction, sessionObject,
					activity, activitydetails,
					"StudyQuestionnaireDAOImpl - updateLineChartSchedule()");
			...
			activity = FdahpStudyDesignerConstants.FORMSTEP_QUESTION_ACTIVITY
					+ " saved.";
			activitydetails = "Content saved for Question of Form Step. (Study ID = "
					+ customStudyId + ")";
			auditLogDAO.saveToAuditLog(session, transaction, sessionObject,
					activity, activitydetails,
					"StudyQuestionnaireDAOImpl - updateLineChartSchedule()");
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - updateLineChartSchedule() - ERROR ",
					e);
		} finally {
			if (null != newSession) {
				newSession.close();
	...
	public String validateLineChartSchedule(Integer questionnaireId,
		...
		try {
			...
			String searchQuery = "select count(*) from questions QBO,questionnaires_steps QSBO where QBO.id=QSBO.instruction_form_id and QSBO.questionnaires_id="
					+ questionnaireId
					+ " and QSBO.active=1 and QSBO.step_type='"
					+ FdahpStudyDesignerConstants.QUESTION_STEP
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - validateLineChartSchedule() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
	...
	public String validateRepetableFormQuestionStats(Integer formId) {
		...
		try {
			...
			BigInteger questionCount = (BigInteger) session.createSQLQuery(
					searchQuery).uniqueResult();
			if (questionCount != null && questionCount.intValue() > 0) {
				message = FdahpStudyDesignerConstants.SUCCESS;
			}
			...
			logger.error(
					"StudyQuestionnaireDAOImpl - validateRepetableFormQuestionStats() - ERROR ",
					e);
		} finally {
			if (session != null) {
				session.close();
