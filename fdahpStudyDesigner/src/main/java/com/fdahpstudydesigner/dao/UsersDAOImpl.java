...
public class UsersDAOImpl implements UsersDAO {
  ...
  public String activateOrDeactivateUser(
    ...
    try {
      ...
      query =
          session.createQuery(
              " UPDATE UserBO SET enabled = :userStatusNew, modifiedOn = now(), "
    ...
    try {
      ...
      if (!"".equals(selectedStudies) && !"".equals(permissionValues)) {
        ...
        if (updateFlag) {
          query =
              session.createSQLQuery(
                  " delete from study_permission where study_id not in (:selectedStudies) "
                      + "and user_id = :userId");
          query.setInteger("userId", userId);
        ...
        for (int i = 0; i < selectedStudy.length; i++) {
          query =
              session.createQuery(
                  " FROM StudyPermissionBO UBO where UBO.studyId = :selectedStudy"
        ...
        String roleName =
            (String)
                session
                    ...
                    .uniqueResult();
        if (StringUtils.isNotEmpty(roleName)) userBO.setRoleName(roleName);
      }
    } catch (Exception e) {
  ...
  public RoleBO getUserRole(int roleId) {
    ...
    try {
      ...
      query =
          session
              .createQuery("SELECT RBO FROM RoleBO RBO WHERE RBO.roleId = :roleId")
