...
public class LoginDAOImpl implements LoginDAO {
   ...
   */
  @Override
  public ChangePasswordResponseBean changePassword(
      Integer userId, String newPassword, String oldPassword) {
  ...
  public UserBO getValidUserByEmail(String email) {
    ...
    try {
      session = hibernateTemplate.getSessionFactory().openSession();
      SQLQuery query =
          session.createSQLQuery("select * from users UBO where BINARY lower(UBO.email) = :email");
      ...
      if (userBo != null) {
        ...
        if (userBo.getRoleId() != null) {
          SQLQuery sqlQuery =
              session.createSQLQuery("select role_name from roles where role_id=:roleId");
          sqlQuery.setInteger("roleId", userBo.getRoleId());
          String role = (String) sqlQuery.uniqueResult();
          if (StringUtils.isNotEmpty(role)) userBo.setRoleName(role);
        }
      }
    } catch (Exception e) {
      userBo = null;
  ...
  public void updateFailAttempts(String userEmailId) {
    ...
    try {
      ...
      SQLQuery query =
          session.createSQLQuery("select * from users UBO where BINARY UBO.email = :email");
      query.setString("email", userEmailId);
      query.addEntity(UserBO.class);
      userBO = (UserBO) query.uniqueResult();
      if (userBO != null) {
  ...
  public void updateToHashedPassword(Integer userId, String hashedPassword, String rawSalt) {
    ...
    try {
      ...
      transaction = session.beginTransaction();
      if (userId != null && userId != 0) {
        UserPasswordHistory passwordHistories =
            (UserPasswordHistory)
                session
                    .createQuery(
