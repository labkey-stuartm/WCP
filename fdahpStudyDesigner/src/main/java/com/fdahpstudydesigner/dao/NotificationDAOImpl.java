...
public class NotificationDAOImpl implements NotificationDAO {
  ...
  public String deleteNotification(
    ...
    try {
      ...
      if (notificationIdForDelete != 0) {
        queryString =
            "update NotificationBO NBO set NBO.modifiedBy = :userId, NBO.modifiedOn = now(), NBO.notificationStatus = 1 ,NBO.notificationDone = 1 ,NBO.notificationAction = 1 where NBO.notificationId = :idForDelete";
  ...
  public List<NotificationHistoryBO> getNotificationHistoryListNoDateTime(int notificationId) {
    ...
    try {
      session = hibernateTemplate.getSessionFactory().openSession();
      queryString =
          "from NotificationHistoryBO NHBO where NHBO.notificationSentDateTime <> null "
  ...
  public Integer saveOrUpdateOrResendNotification(
    ...
    try {
      ...
      if (notificationBO.getNotificationId() == null) {
        ...
        query =
            session.createQuery(
                " from NotificationBO NBO where NBO.notificationId = :notificationId");
