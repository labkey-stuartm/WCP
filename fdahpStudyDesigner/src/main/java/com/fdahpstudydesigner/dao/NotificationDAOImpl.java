...
public class NotificationDAOImpl implements NotificationDAO {
	...
	public String deleteNotification(int notificationIdForDelete,
		...
		try {
			...
			if (notificationIdForDelete != 0) {
				queryString = "update NotificationBO NBO set NBO.modifiedBy = "
	...
	public NotificationBO getNotification(int notificationId) {
		...
		try {
			session = hibernateTemplate.getSessionFactory().openSession();
			queryString = "from NotificationBO NBO where NBO.notificationId = "
					+ notificationId;
			query = session.createQuery(queryString);
			notificationBO = (NotificationBO) query.uniqueResult();
			if (null != notificationBO) {
				notificationBO.setNotificationId(null != notificationBO
						.getNotificationId() ? notificationBO
						.getNotificationId() : 0);
				notificationBO.setNotificationText(null != notificationBO
						.getNotificationText() ? notificationBO
						.getNotificationText() : "");
				notificationBO.setScheduleDate(null != notificationBO
						.getScheduleDate() ? notificationBO.getScheduleDate()
						: "");
				notificationBO.setScheduleTime(null != notificationBO
						.getScheduleTime() ? notificationBO.getScheduleTime()
						: "");
				notificationBO.setNotificationSent(notificationBO
						.isNotificationSent());
				notificationBO
						.setNotificationScheduleType(null != notificationBO
								.getNotificationScheduleType() ? notificationBO
								.getNotificationScheduleType() : "");
				if (FdahpStudyDesignerConstants.NOTIFICATION_IMMEDIATE
						.equalsIgnoreCase(notificationBO
								.getNotificationScheduleType())) {
	...
	public List<NotificationHistoryBO> getNotificationHistoryListNoDateTime(
		...
		try {
			...
			logger.error(
					"NotificationDAOImpl - getNotificationHistoryListNoDateTime - ERROR",
					e);
		} finally {
			if (null != session) {
				session.close();
	...
	public List<NotificationBO> getNotificationList(int studyId, String type) {
		...
		try {
			...
			logger.error("NotificationDAOImpl - getNotificationList() - ERROR",
					e);
		} finally {
			if (null != session) {
				session.close();
	...
	public List<PushNotificationBean> getPushNotificationList(String date,
		...
		try {
			...
			sb.append(
					...
					.append(FdahpStudyDesignerConstants.NOTIFICATION_GT)
					.append("' OR  s.status = '")
			...
			if (null != pushNotificationBeans
				...
				for (PushNotificationBean pushNotificationBean : pushNotificationBeans) {
					notificationIds.add(pushNotificationBean
							.getNotificationId());
					if (pushNotificationBean.getNotificationSubType() == null
							|| (pushNotificationBean.getNotificationSubType() != null
									&& !FdahpStudyDesignerConstants.RESOURCE
											.equals(pushNotificationBean
													.getNotificationSubType()) && !FdahpStudyDesignerConstants.STUDY_EVENT
										.equals(pushNotificationBean
												.getNotificationSubType()))) {
						NotificationHistoryBO historyBO = new NotificationHistoryBO();
						historyBO.setNotificationId(pushNotificationBean
								.getNotificationId());
						historyBO
								.setNotificationSentDateTime(FdahpStudyDesignerUtil
										.getCurrentDateTime());
			...
			if (null != trans)
				trans.rollback();
			logger.error(
					"NotificationDAOImpl - getPushNotificationList - ERROR", e);
		} finally {
			if (null != session)
				session.close();
		}
	...
	public Integer saveOrUpdateOrResendNotification(
		...
		try {
			...
			if (notificationBO.getNotificationId() == null) {
				...
				notificationBOUpdate.setNotificationText(notificationBO
						.getNotificationText().trim());
				notificationBOUpdate
						.setCreatedBy(notificationBO.getCreatedBy());
				notificationBOUpdate
						.setCreatedOn(notificationBO.getCreatedOn());
				notificationBOUpdate.setNotificationScheduleType(notificationBO
						.getNotificationScheduleType());
				if ("".equals(notificationBO.getScheduleTime())) {
					notificationBOUpdate.setScheduleTime(null);
				} else {
					notificationBOUpdate.setScheduleTime(notificationBO
							.getScheduleTime());
				}
				if ("".equals(notificationBO.getScheduleDate())) {
					notificationBOUpdate.setScheduleDate(null);
				} else {
					notificationBOUpdate.setScheduleDate(notificationBO
							.getScheduleDate());
				}

				if (notificationType
						.equals(FdahpStudyDesignerConstants.STUDYLEVEL)) {
					notificationBOUpdate.setNotificationDone(notificationBO
							.isNotificationDone());
					notificationBOUpdate
							.setNotificationType(FdahpStudyDesignerConstants.NOTIFICATION_ST);
					notificationBOUpdate.setCustomStudyId(notificationBO
							.getCustomStudyId());
					notificationBOUpdate
							.setStudyId(notificationBO.getStudyId());
					notificationBOUpdate.setNotificationAction(notificationBO
				...
				query = session
						.createQuery(" from NotificationBO NBO where NBO.notificationId = "
								+ notificationBO.getNotificationId());
				notificationBOUpdate = (NotificationBO) query.uniqueResult();

				if (StringUtils
						.isNotBlank(notificationBO.getNotificationText())) {
					notificationBOUpdate.setNotificationText(notificationBO
							.getNotificationText().trim());
				} else {
					notificationBOUpdate
							.setNotificationText(notificationBOUpdate
									.getNotificationText().trim());
				}
				notificationBOUpdate.setModifiedBy(notificationBO
						.getModifiedBy());
				notificationBOUpdate.setModifiedOn(notificationBO
						.getModifiedOn());
				notificationBOUpdate.setCustomStudyId(notificationBO
						.getCustomStudyId());
				notificationBOUpdate.setStudyId(notificationBOUpdate
						.getStudyId());
				notificationBOUpdate.setNotificationSent(notificationBO
						.isNotificationSent());
				notificationBOUpdate.setNotificationScheduleType(notificationBO
						.getNotificationScheduleType());
				if (FdahpStudyDesignerUtil.isNotEmpty(notificationBO
						.getScheduleTime())) {
					notificationBOUpdate.setScheduleTime(notificationBO
							.getScheduleTime());
				} else {
				...
				if (notificationType
					...
					notificationBOUpdate
							.setNotificationDone(notificationBOUpdate
									.isNotificationDone());
					notificationBOUpdate
							.setNotificationType(FdahpStudyDesignerConstants.NOTIFICATION_GT);
					notificationBOUpdate
							.setNotificationAction(notificationBOUpdate
									.isNotificationAction());
				}
				notificationBOUpdate
						.setNotificationSubType(FdahpStudyDesignerConstants.NOTIFICATION_SUBTYPE_ANNOUNCEMENT);
			...
			if (notificationId != null) {
				...
				if ("add".equals(buttonType)) {
					activitydetails = "Gateway (app-level) notification created. (Notification ID = "
							+ notificationId + ")";
				} else if ("update".equals(buttonType)) {
					activitydetails = "Gateway (app-level) notification updated. (Notification ID = "
							+ notificationId + ")";
				} else if ("resend".equals(buttonType)
						&& !notificationType
								.equals(FdahpStudyDesignerConstants.STUDYLEVEL)) {
					activitydetails = "Gateway (app-level) notification resend. (Notification ID = "
							+ notificationId + ")";
				} else if ("resend".equals(buttonType)
						&& notificationType
								.equals(FdahpStudyDesignerConstants.STUDYLEVEL)) {
					...
					activitydetails = "Notification content saved. (Study ID = "
							+ notificationBO.getCustomStudyId()
							+ ", Notification ID = " + notificationId + ")";
				...
				auditLogDAO
						.saveToAuditLog(session, transaction, sessionObject,
								notificationType, activitydetails,
			...
			transaction.rollback();
			logger.error(
					"NotificationDAOImpl - saveOrUpdateOrResendNotification() - ERROR",
					e);
		} finally {
			if (null != session) {
				session.close();
