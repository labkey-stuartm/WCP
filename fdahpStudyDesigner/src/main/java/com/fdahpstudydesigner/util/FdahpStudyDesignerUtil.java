...
public class FdahpStudyDesignerUtil {
  ...
  /**
   * This method is used to build user's spring security authorities from it's granted permission
   *
   * @author BTC
  ...
  /**
   * This method used to map the dynamic value to it's static email marker to create a dynamic email
   * contain
   *
   * @author BTC
  ...
  /**
   * This methods read all property from properties file by using {@link ResourceBundle} and add it
   * to a map
   *
    ...
    if (lhs.contains("x")) {
      if (lhs.contains("!=")
        ...
        try {
          ...
          if (oprandResult.intValue() == 1) operand1 = "true";
          else operand1 = "false";
        } catch (Exception e) {
    ...
    if (rhs.contains("x")) {
      if (rhs.contains("!=")
        ...
        try {
          ...
          if (oprandResult.intValue() == 1) operand2 = "true";
          else operand2 = "false";
        } catch (Exception e) {
          logger.error("FdahpStudyDesignerUtil - getConditionalFormulaResult() : ", e);
          formulaInfoBean.setStatusMessage("Error in LHS");
  ...
  /**
   * Return Current Date and Time as {@link String} with yyyy-MM-dd HH:mm:ss format
   *
   * @return {@link String}
   */
  ...
  /**
   * Return last exception in spring security authentication for current {@link HttpServletRequest}
   *
   * @author BTC
