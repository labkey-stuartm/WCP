...
@NamedQueries({
  @NamedQuery(
      name = "getConsentByStudyId",
      query = " From ConsentBo CBO WHERE CBO.studyId =:studyId order by CBO.createdOn DESC"),
  @NamedQuery(
      name = "updateStudyConsentVersion",
      query = "UPDATE ConsentBo SET live=2 WHERE customStudyId=:customStudyId and live=1"),
})
public class ConsentBo implements Serializable {
  ...
  @Transient private String comprehensionTest;
  ...
  @Transient private String[] signatures = new String[] {};
  ...
  @Transient private boolean reviewAndEconsentPage;
