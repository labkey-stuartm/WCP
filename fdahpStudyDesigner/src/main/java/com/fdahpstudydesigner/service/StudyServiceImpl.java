...
public class StudyServiceImpl implements StudyService {
	...
	public boolean copyliveStudyByCustomStudyId(String customStudyId, SessionObject object) {
		...
		boolean flag = false;
		try {
			flag = studyDAO.resetDraftStudyByCustomStudyId(customStudyId, FdahpStudyDesignerConstants.COPY_STUDY,
					object);
		} catch (Exception e) {
	...
	public String deleteEligibilityTestQusAnsById(Integer eligibilityTestId, Integer studyId,
		...
		try {
			message = studyDAO.deleteEligibilityTestQusAnsById(eligibilityTestId, studyId, sessionObject,
					customStudyId);
		} catch (Exception e) {
	...
	public String deleteResourceInfo(Integer resourceInfoId, SessionObject sesObj, String customStudyId, int studyId) {
		...
		try {
			...
			if (message.equals(FdahpStudyDesignerConstants.SUCCESS)) {
				...
				activityDetail = "Resource has been soft-deleted from the Study. (Study ID = " + customStudyId
						+ ", Resource Display Title = " + resourceBO.getTitle() + ").";
				auditLogDAO.saveToAuditLog(null, null, sesObj, activity, activityDetail,
						"StudyDAOImpl - deleteResourceInfo()");
			}
		} catch (Exception e) {
			logger.error("StudyServiceImpl - deleteConsentInfo() - Error", e);
		}
	...
	public ConsentInfoBo getConsentInfoById(Integer consentInfoId) {
		...
		try {
			...
			if (consentInfoBo != null) {
				consentInfoBo
						.setBriefSummary(consentInfoBo.getBriefSummary().replaceAll("(\\r|\\n|\\r\\n)+", "&#13;&#10;"));
			}
		} catch (Exception e) {
	 ...
	 * @exception Exception
	 *
	 *                      This method return content of Overview pages of the
	 *                      Study those pages shows in mobile side as a set of
	 *                      swipe-able screens that carry information about the
	 *                      study, with each screen having A title Description Image
	 *
	 */
	@Override
	...
	public ResourceBO getResourceInfo(Integer resourceInfoId) {
		...
		try {
			resourceBO = studyDAO.getResourceInfo(resourceInfoId);
			if (null != resourceBO) {
				resourceBO.setStartDate(FdahpStudyDesignerUtil.isNotEmpty(resourceBO.getStartDate())
						? String.valueOf(FdahpStudyDesignerUtil.getFormattedDate(resourceBO.getStartDate(),
								FdahpStudyDesignerConstants.DB_SDF_DATE, FdahpStudyDesignerConstants.UI_SDF_DATE))
						: "");
				resourceBO.setEndDate(FdahpStudyDesignerUtil.isNotEmpty(resourceBO.getEndDate())
						? String.valueOf(FdahpStudyDesignerUtil.getFormattedDate(resourceBO.getEndDate(),
								FdahpStudyDesignerConstants.DB_SDF_DATE, FdahpStudyDesignerConstants.UI_SDF_DATE))
						: "");
			}
		} catch (Exception e) {
	 ...
	 * @author BTC
	 * @param Integer : studyId
	 * @param String  : markCompleted can be
	 *                consent/consentreview/questionnaire/comprehenstionTest
	 * @param Boolean : flag(true/false)
	 * @param Object  : {@link SessionObject}
	 * @param String  : customStudyId in {@link StudyBo}
	 * @return String : {SUCCES/FAILURE} Description : This method is to make mark
	 *         as completed of study
	 */
	@Override
	...
	public boolean resetDraftStudyByCustomStudyId(String customStudy) {
		...
		try {
			...
			flag = studyDAO.resetDraftStudyByCustomStudyId(customStudy, FdahpStudyDesignerConstants.RESET_STUDY,
					object);
			if (flag)
	...
	public Integer saveOrDoneChecklist(Checklist checklist, String actionBut, SessionObject sesObj,
		...
		try {
			...
			if (!checklistId.equals(0)) {
				...
				auditLogDAO.saveToAuditLog(null, null, sesObj, activity, activityDetail,
						"StudyDAOImpl - saveOrDoneChecklist()");
			}
		} catch (Exception e) {
			...
			if (comprehensionTestQuestionBo != null) {
				if (comprehensionTestQuestionBo.getId() != null) {
					updateComprehensionTestQuestionBo = studyDAO
							.getComprehensionTestQuestionById(comprehensionTestQuestionBo.getId());
				} else {
				...
				if (comprehensionTestQuestionBo.getStructureOfCorrectAns() != null) {
					updateComprehensionTestQuestionBo
							.setStructureOfCorrectAns(comprehensionTestQuestionBo.getStructureOfCorrectAns());
				}
	...
	public Integer saveOrUpdateEligibilityTestQusAns(EligibilityTestBo eligibilityTestBo, Integer studyId,
		...
		try {
			if (eligibilityTestBo != null) {
				...
				eligibilityTestId = studyDAO.saveOrUpdateEligibilityTestQusAns(eligibilityTestBo, studyId,
				...
				for (int i = 0; i < studyPageBean.getMultipartFiles().length; i++) {
					...
					if (!studyPageBean.getMultipartFiles()[i].isEmpty()) {
						if (FdahpStudyDesignerUtil.isNotEmpty(studyPageBean.getImagePath()[i])) {
							file = studyPageBean.getImagePath()[i]
									.replace(
											"." + studyPageBean.getImagePath()[i].split(
													"\\.")[studyPageBean.getImagePath()[i].split("\\.").length - 1],
											"");
						} else {
							file = FdahpStudyDesignerUtil.getStandardFileName("STUDY_PAGE_" + i,
									studyPageBean.getUserId() + "", studyPageBean.getStudyId());
						}
						imagePath[i] = FdahpStudyDesignerUtil.uploadImageFile(studyPageBean.getMultipartFiles()[i],
								file, FdahpStudyDesignerConstants.STUDTYPAGES);
					} else {
	...
	public Integer saveOrUpdateResource(ResourceBO resourceBO, SessionObject sesObj) {
		...
		try {
			...
			if (!resourceBO.isAction()) {
				...
				activityDetails = "Resource content saved. (Study ID = " + studyBo.getCustomStudyId()
						+ ", Resource Display Title = " + resourceBO.getTitle() + ").";
			} else {
				...
				activityDetails = "Resource successfully checked for minimum content completeness and marked 'Done'. (Study ID = "
						+ studyBo.getCustomStudyId() + ", Resource Display Title = " + resourceBO.getTitle() + ").";
			}
			...
			if (resourceBO.getPdfFile() != null && !resourceBO.getPdfFile().isEmpty()) {
				file = FdahpStudyDesignerUtil.getStandardFileName(
						...
						sesObj.getFirstName(), sesObj.getLastName()).replaceAll("\\W+", "_");
				fileName = FdahpStudyDesignerUtil.uploadImageFile(resourceBO.getPdfFile(), file,
			...
			resourceBO2
					.setResourceText(null != resourceBO.getResourceText() ? resourceBO.getResourceText().trim() : "");
			...
			resourceBO2
					.setStartDate(FdahpStudyDesignerUtil.isNotEmpty(resourceBO.getStartDate())
							? String.valueOf(FdahpStudyDesignerUtil.getFormattedDate(resourceBO.getStartDate(),
									FdahpStudyDesignerConstants.UI_SDF_DATE, FdahpStudyDesignerConstants.DB_SDF_DATE))
							: null);
			resourceBO2
					.setEndDate(FdahpStudyDesignerUtil.isNotEmpty(resourceBO.getEndDate())
							? String.valueOf(FdahpStudyDesignerUtil.getFormattedDate(resourceBO.getEndDate(),
									FdahpStudyDesignerConstants.UI_SDF_DATE, FdahpStudyDesignerConstants.DB_SDF_DATE))
							: null);
				...
				if (resourceBO.isAction()) {
					...
					if (!resourceBO2.isStudyProtocol()) {
						...
						if (studyBo.getLiveStudyBo() != null) {
							...
							saveNotiFlag = true;
							if(!StringUtils.equalsIgnoreCase(FdahpStudyDesignerConstants.STUDY_LANGUAGE_SPANISH, studyBo.getStudyLanguage())) {
								notificationText = "Study Protocol information has been " + innerText + " for the study "
										+ studyName + ". Visit the app to read it now.";
							}else {
								if(StringUtils.equalsIgnoreCase("updated", innerText)) {
									notificationText=SpanishLangConstants.STUDY_PROTOCOL_NOTIFICATION_TEXT_UPDATED.replace("$studyName", studyName);
								}else {
									notificationText=SpanishLangConstants.STUDY_PROTOCOL_NOTIFICATION_TEXT_ADDED.replace("$studyName", studyName);
								}
							}
						}
					}
	...
	public ParticipantPropertiesBO getParticipantProperty(String participantPropertyId, String customStudyId) {
		...
		try {
			participantPropertiesBO = studyDAO.getParticipantProperty(participantPropertyId, customStudyId);
		} catch (Exception e) {
	...
	/**
	 * This method is to validate Study eligible for launch/publish based on some
	 * condition
	 * 
	 * @author BTC
	 * @param string , studyId
